<?php
/*
 * @version $Id: transfer.class.php 19509 2012-10-16 10:45:43Z moyo $
 -------------------------------------------------------------------------
...
//                                 'Reminder', 'Software', 'SoftwareLicense', 'Supplier','Ticket','User');
   ...
   **/
   function moveItems($items, $to, $options) {
	...
	/*//SUPRISERVICE*/
   function logPrinterTransfer($ID, $to) {
      global $DB, $CFG_GLPI;
		$query = "INSERT
					 INTO `supridesk_log_printer_entity`
					 (`printers_id`, `date`, `entity_id_from`, `entity_id_to`)
					SELECT $ID, CURRENT_TIMESTAMP(), entities_id, $to from glpi_printers where id = $ID";
		$DB->query($query);
	}						
	//--
         ...
         if ($DB->numrows($result) != 0) {
            switch ($this->options['keep_ticket']) {
               ...
               case 1 :
                  ...
                  /*//SUPRISERVICE //Alterações Marcelo mantem os chamados nas entidades quando tranferir
                  while ($data = $DB->fetch_array($result)) {
