<?php
/*
 * @version $Id: ticketplanning.class.php 13811 2011-02-07 10:27:48Z yllen $
 -------------------------------------------------------------------------
...
//                  if (haveAccessToEntity($job->fields["entities_id"])) {
                     ...
                     $interv[$data["begin"]."$$$".$i]["content"]  = resume_text($job->fields["content"],
                                                                                $CFG_GLPI["cut"]);
                     $interv[$data["begin"]."$$$".$i]["device"]   = ($job->hardwaredatas
                                                                     ?$job->hardwaredatas->getName()
                                                                     :'');
...
//                  }
   ...
   **/
   static function getAlreadyPlannedInformation($val) {
      ...
      $out  = Ticket::getTypeName().' : '.convDateTime($val["begin"]).' -> '.
   ...
   static function displayPlanningItem($val, $who, $type="", $complete=0) {
      ...
      if ($complete) {
         ...
         $content = "<strong>".Planning::getState($val["state"])."<br>".
                    $LANG['joblist'][2]."&nbsp;:</strong> ".Ticket::getPriorityName($val["priority"]).
