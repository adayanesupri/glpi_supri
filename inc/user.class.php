...
/*
 * @version $Id: user.class.php 19387 2012-10-02 14:22:16Z moyo $
 -------------------------------------------------------------------------
...
class User extends CommonDBTM {
   ...
   **/
   ...
   function defineTabs($options=array()) {
      ...
      $this->addStandardTab('Profile_User', $ong, $options);
		/*//SUPRISERVICE*/
      if ((Session::haveRight("cartridge_kits","r")) || (Session::haveRight("cartridge_kits","w"))) {
         ...
         if (empty($input["password"])) {
            ...
            if ($input["password"]==$input["password2"]) {
               ...
               unset($input["password2"]);
					...
					/*//SUPRISERVICE*/
					if ( isset($_SESSION["glpiForcePasswordChange"]) && $_SESSION["glpiForcePasswordChange"] == 1) {
						$samePassword = ( $this->fields['password'] == $input["password"] );
						if ( $samePassword )
						{
							Session::addMessageAfterRedirect($LANG['supriservice'][3], false, ERROR);
							return false;
						}
						unset($_SESSION["glpiForcePasswordChange"]);
   ...
   **/
   function showForm($ID, $options=array()) {
      ...
      if ($this->fields["name"] == "") {
         ...
         echo "<td><input id='validate_name' name='name' value='" . $this->fields["name"] . "'>";
         $parameters = array('login' => '__VALUE__');
			Ajax::updateItemOnEvent('validate_name', 'validate_login', $CFG_GLPI["root_doc"]."/ajax/userLoginCheck.php",
					  $parameters, array("blur"));
			echo "<span id='validate_login'></span>";
         //echo "<img alt='' title='Verificar login' src='".$CFG_GLPI["root_doc"]. "/pics/actualiser.png' style='cursor:pointer; margin-left:2px;'>";
			echo "</td>";
      ...
      if (Session::haveRight("user", "w")) {
         if ((!$extauth || empty($ID))
            ...
            echo "<td><input type='password' name='password' value='' size='20' autocomplete='off'>";
				/*//SUPRISERVICE*/
				echo "<span>&nbsp;&nbsp;&nbsp;";
				echo $LANG['supriservice'][4]."&nbsp;:";
				Dropdown::showYesNo('force_password_change',$this->fields['force_password_change']);
				echo "</span>";
				//--
            echo "</td></tr>";
         } else {
      ...
      if (Session::haveRight("user", "w")) {
		...
		/*//SUPRISERVICE*/
      echo "<tr class='tab_bg_1'><td>IMEI do Dispositivo:</td><td>";
      Html::autocompletionTextField($this, "mobidesk_code");
      echo "</td>";
      echo "<td colspan='2'>&nbsp;</td></tr>";
		//--
     ...
     //Authentications informations : auth method used and server used
      ...
      echo "<tr class='tab_bg_1'><td>" . $LANG['users'][26] . "&nbsp;:</td>";
             echo "<td><input type='text' name='value_time' value='".
                  Html::formatNumber($this->fields["value_time"], true)."' size='14'></td></tr>";
   ...
   function showMyForm($target, $ID) {
      ...
      if ($this->getFromDB($ID)) {
         ...
         //do some rights verification
			...
			/*//SUPRISERVICE*/
         if ((!$extauth && Session::haveRight("password_update", "1")) || $forcePasswordChange ) {
            echo "<td>" . $LANG['setup'][19] . "&nbsp;:</td>";
   ...
   **/
   ...
   function pre_updateInDB() {
      global $DB,$LANG;
		...
		/*//SUPRISERVICE*/
      if (($key=array_search('mobidesk_code',$this->updates)) === false) {
			if (($key=array_search('mobidesk_code_mod',$this->updates)) !== false) {
				unset($this->updates[$key]);
			}

			/*
			print_r($this->updates);
			print "<br><br>teste<br>";
			print_r($this->fields);
			print "<br>" . $this->input['mobidesk_code'];
			print "<br><br><br><br>";
			print_r($this->oldvalues);
			die();
			*/
		}
		//--
   ...
   **/
   ...
   static function getNameUser($user_id){
       global $DB;
       
       $query = "SELECT CONCAT(`firstname`,' ',`realname`) AS name
                 FROM `glpi_users` 
                 WHERE `id` = {$user_id}";
       $result = $DB->query($query);
