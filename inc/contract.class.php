<?php
/*
 * @version $Id: contract.class.php 18771 2012-06-29 08:49:19Z moyo $
 -------------------------------------------------------------------------
...
class Contract extends CommonDBTM {
   ...
   public $dohistory = true;
	/*//SUPRISERVICE*/
   protected $forward_entity_to = array('Contract_Item_Supridesk', 'Agrupamento_Supridesk', 'Tarifacao_Supridesk');
   ...
   function cleanDBonPurge() {
      ...
      $ci->cleanDBonItemDelete($this->getType(), $this->fields['id']);
		...
		/*//SUPRISERVICE*/
      $cis = new Contract_Item_Supridesk();
      $cis->cleanDBonItemDelete($this->getType(), $this->fields['id']);
   }
   ...
   function defineTabs($options=array()) {
      ...
      $this->addStandardTab('Contract_Item_Supridesk', $ong, $options);
      $this->addStandardTab('Agrupamento_Supridesk', $ong, $options);
      $this->addStandardTab('Tarifacao_Supridesk', $ong, $options);
      $this->addStandardTab('Supridesk_Faturamento', $ong, $options);
   ...
   **/
   function showForm($ID,$options=array()) {
      ...
      echo "<td>";
      echo "<textarea cols='50' rows='4' name='comment' >".$this->fields["comment"]."</textarea></td>";
   ...
   static function getSearchOptionsToAdd() {
	 ...
	 /* $tab[139]['table']         = 'glpi_contracts';
      $tab[139]['field']         = 'is_active';
      $tab[139]['name']          = "Ativo";
      $tab[139]['forcegroupby']  = true;
	  $tab[139]['datatype']      = 'number';
      $tab[139]['massiveaction'] = false;
      $tab[139]['joinparams']    = $joinparams;*/
   ...
   function getSearchOptions() {
	  ...
	  $tab[140]['table']         = $this->getTable();
      $tab[140]['field']         = 'is_active';
      $tab[140]['name']          = "Ativo";
      $tab[140]['datatype'] = 'bool';
      //$tab[140]['searchtype'] = 'equals';
      $tab[140]['massiveaction'] = false;
   ...
   static function cronContract($task=NULL) {
      ...
      foreach (Entity::getEntitiesToNotify('use_contracts_alert') as $entity => $value) {
         ...
         $querys = array(Alert::NOTICE => $query_notice,
                         Alert::END    => $query_end);
   ...
   **/
   static function dropdown($options=array()) {
      ...
      $idrest = "";
      if ($p['entity']>=0) {
         $entrest = getEntitiesRestrictRequest("AND", "glpi_contracts", "entities_id", $p['entity'],
                                               true);
      }
      if (count($p['used'])) {
      ...
      while ($data=$DB->fetch_array($result)) {
         if ($p['nochecklimit']
            ...
            echo Toolbox::substr($data["name"]." - #".$data["num"]." - ".
                                 Html::convDateTime($data["begin_date"]), 0,
   ...
   **/
   ...
   function getUnallowedFieldsForUnicity() {
	...
	/*//SUPRIDESK*/
   static function showAssociatedPrinter(Printer $item, $withtemplate='') {
      global $DB, $CFG_GLPI, $LANG;
      ...
      $canedit = $item->can($ID,"w");
		...
		$query = " select c.id as contracts_id,
								c.name as contracts_name,
								ci.id as contracts_items_id,
								ci.nome as contracts_items_name
					  from supridesk_contracts_items_printers cip, supridesk_contracts_items ci, glpi_contracts c
      ...
      while ($i < $number) {
         ...
         $ciID = $DB->result($result, $i, "contracts_items_id");
         $ciName = $DB->result($result, $i, "contracts_items_name");
         echo "<td class='center b'>";
         echo "<a href='".$CFG_GLPI["root_doc"]."/front/contract_item_supridesk.form.php?id=$ciID'>".$ciName;
         if ($_SESSION["glpiis_ids_visible"] || empty($ciName)) {
            echo " (".$ciID.")";
         }
         echo "</a></td>";
			...
			/*
         echo "<td class='center'>";
         echo $DB->result($result, $i, "contracts_items_name")."</td>";
			*/
