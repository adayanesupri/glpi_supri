<?php
/*
 * @version $Id: commondbtm.class.php 19104 2012-08-21 07:43:03Z moyo $
 -------------------------------------------------------------------------
...
class CommonDBTM extends CommonGLPI {
   ...
   **/
   function addToDB() {
      ...
      $nb_fields = count($this->fields);
       ...
       if(isset($_SESSION['itemtypedoc'])){
           $this->fields['itemtype'] = 'Doc';
           unset($_SESSION['itemtypedoc']);
       }
   ...
   function canUnrecurs() {
      ...
      if (isset($RELATION[$this->getTable()])) {
         foreach ($RELATION[$this->getTable()] as $tablename => $field) {
            ...
            /*//SUPRISERVICE OSWALDO: MARRETA, REMOVER NO FUTURO */
            $itemtype = getItemTypeForTable($tablename);
            if ($itemtype == "UNKNOWN") return true;
   ...
   function showFormHeader($options=array()) {
      ...
      if (isset($this->fields["entities_id"])
          ...
          && $this->isEntityAssign()) {
			...
			/*//SUPRISERVICE*/
			echo "&nbsp;(";
			if ( $this->fields["entities_id"] != $_SESSION['glpiactive_entity'] )
			{
				$target = $CFG_GLPI['root_doc']."/front/central.php";
				echo "<a href='".$target."?active_entity=".$this->fields["entities_id"]."&amp;is_recursive=1'>
							<img src='".$CFG_GLPI["root_doc"]."/pics/greenbutton.png"."' border=0 title='Alternar para esta entidade'>
					</a>";
			}
   ...
   **/
   function can($ID, $right, &$input=NULL) {
      ...
      /* //SUPRISERVICE
         OSWALDO: Analisar isso no futuro:
	      Passa o id do tecnico para o id do usuario proprietario do item
      */
      if ( isset($this->fields['users_id_tech']) && !isset($this->fields['users_id']) ) $this->fields['users_id'] = $this->fields['users_id_tech'];
   ...
   **/
   function getComments() {
      ...
      if ($this->isField('otherserial')) {
		...
		/*//SUPRISERVICE*/
		$modelo = "N/D";
      if ($this->isField('printermodels_id')) {
			$model = new PrinterModel();
			$model->getFromDB( $this->getField('printermodels_id') );
			$modelo = array_key_exists('name', $model->fields) ? $model->fields['name'] : 'N/D';
		}
      else if ($this->isField('computermodels_id')) {
			$model = new ComputerModel();
			$model->getFromDB( $this->getField('computermodels_id') );
			$modelo = array_key_exists('name', $model->fields) ? $model->fields['name'] : 'N/D';
		}
      else if ($this->isField('peripheralmodels_id')) {
			$model = new PeripheralModel();
			$model->getFromDB( $this->getField('peripheralmodels_id') );
			$modelo = array_key_exists('name', $model->fields) ? $model->fields['name'] : 'N/D';
		}
      else if ($this->isField('monitormodels_id')) {
			$model = new MonitorModel();
			$model->getFromDB( $this->getField('monitormodels_id') );
			$modelo = array_key_exists('name', $model->fields) ? $model->fields['name'] : 'N/D';
		}

		$comment .= "<span class='b'>".$LANG['common'][22]."&nbsp;: </span>".$modelo."<br>";
		/*//END SUPRISERVICE*/
