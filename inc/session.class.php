<?php
/*
 * @version $Id: session.class.php 18843 2012-07-04 18:52:24Z moyo $
 -------------------------------------------------------------------------
         ...
         if (isset($auth->user->fields['id'])
            ...
            if (!$auth->user->fields['is_deleted'] && $auth->user->fields['is_active']) {
               ...
               $_SESSION["glpicrontimer"]       = time();
					/*//SUPRISERVICE*/
               $_SESSION["glpiForcePasswordChange"] = $auth->user->fields['force_password_change'];
               // Default tab
//               $_SESSION['glpi_tab']=1;
   ...
   **/
   static function getLoginUserID($force_human=true) {
	...
	/*//SUPRISERVICE*/
	//Verifica se a opção de alterar senha forçada está marcada e se já não está na tela de preferências.
   static function getForcePasswordChange() {

      if ( strpos($_SERVER['PHP_SELF'], 'preference.php') ) {
			return false;
      }

      if ( !isset($_SESSION["glpiForcePasswordChange"]) || $_SESSION["glpiForcePasswordChange"] == 0 ) {
         return false;
      }
      else if ( $_SESSION["glpiForcePasswordChange"] == 1 ) {
			return true;
		}
      return false;
   }
               ...
               switch ($message_type) {
                  case ERROR :
                     ...
                     break;
						/*//SUPRISERVICE*/
                  case WARNING :
							$pics = $CFG_GLPI["root_doc"]."/pics/warning.png";
                     $_SESSION["MESSAGE_AFTER_REDIRECT"] .= "<img src='".$pics."' alt=\"".$pics."\" title=\"".$pics."\"><h3><span class='orange'>$toadd</span></h3>";
                     break;
