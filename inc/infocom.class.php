<?php
/*
 * @version $Id: infocom.class.php 18771 2012-06-29 08:49:19Z moyo $
 -------------------------------------------------------------------------
...
class Infocom extends CommonDBChild {
   ...
   **/
   function getFromDBforDevice ($itemtype, $ID) {
    ...
    static function getLeasing ($ID) {        
      global $DB;        

      $query = "SELECT *
                FROM `supridesk_leasing`
   ...
   static function showForItem(CommonDBTM $item, $withtemplate='') {
      ...
      if (!$item) {
         ...
         if (!$ic->getFromDBforDevice($item->getType(),$dev_ID)) {
            ...
            $inf = new Infocom();       
            $data_leasing = $inf->getLeasing($dev_ID);
            
            echo "<tr class='tab_bg_1'>";
            echo "<td>Leasing&nbsp;:</td>";
            echo "<td><b>Período :</b>";
            echo $data_leasing['periodo']."&nbsp;&nbsp;&nbsp;&nbsp;<b>Fator: </b>".$data_leasing['fator'];
            /*echo "<img alt='' title='Adicionar' src='/supridesk/pics/add_dropdown.png' style='cursor:pointer; margin-left:2px;' onclick='var w = window.open('/supridesk/front/leasing.form.php?popup=1&amp;rand=1978248701' ,'glpipopup', 'height=36, width=36, top=100, left=100, scrollbars=yes' );w.focus();'>";*/
            echo "</td></tr>";
            echo "<td colspan='2'>";
            ...
            echo "<tr><th colspan='4'>Cadastro de Leasing</th></tr>";
            echo "<form method='post' action=\"".$CFG_GLPI["root_doc"]."/front/stock.form.php\">";  
             echo "<tr class='tab_bg_1'>";
            echo "<td>Fator&nbsp;:</td>";
            echo "<td>";
            Html::autocompletionTextField($ic, "fator", array('size' => 15));
            echo "</td>";
             echo "<td>Período&nbsp;:</td>";
            echo "<td>";
            Html::autocompletionTextField($ic, "periodo",array('size' => 15));
            echo "</td></tr>";
             
            echo "<tr><td class='center tab_bg_2' colspan = '4'>";     
            echo "<input type='hidden' name='tID' value={$_REQUEST['id']}>\n"; 
            echo "<input type='submit' name='add_leasing' value=\"".$LANG['buttons'][8]."\"
           class='submit'></td></tr>";
            echo "</table>";
             Html::closeForm();
         }
...
//                }
   ...
   **/
   static function getWarrantyExpir($from, $addwarranty, $deletenotice=0) {
    ...
    static function add_leasing($periodo,$fator,$equipamento) {
      global $DB;
       
      $query = "INSERT INTO `supridesk_leasing` (`equipamento_id`,`fator`,`periodo`) VALUES ({$equipamento},'{$fator}',{$periodo})";
