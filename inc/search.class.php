<?php
/*
 * @version $Id: search.class.php 19088 2012-08-10 16:02:22Z remi $
 -------------------------------------------------------------------------
         ...
         */
         ...
         if ($p['start']<$numrows) {
            ...
            while ($i<$numrows && $i<($end_display)) {
               ...
               foreach ($toview as $key => $val) {
                   //var_export(self::giveItem($itemtype, $val, $data, $key));
            ...
            if ($output_type==HTML_OUTPUT) {
               if ($showmassiveactions) {
                  ...
                  if ($max>0 && $max<($row_num+10)) {
                     ...
                     echo $LANG['search'][5]."</span>";
                     if ($_SESSION['glpi_use_mode'] == Session::DEBUG_MODE) {
                        echo "<br>".$LANG['search'][6];
                     }
                     echo "</td></tr></table>";
                  } else {
      ...
      switch ($table.".".$field) {
         ...
         case "glpi_complete_entities.completename" :
            if ($itemtype == 'User' && $ID == 80) {
					global $LANG;
					/*//SUPRISERVICE*/
					$completeName = "IF( glpi_profiles_users.entities_id <= 0, 
											'".addslashes($LANG['entity'][2])."', 
											GROUP_CONCAT(`$table$addtable`.`completename` SEPARATOR '$$$$'))
											AS ".$NAME."_$num,";
