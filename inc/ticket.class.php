...
/*
 * @version $Id: ticket.class.php 19487 2012-10-11 18:57:36Z moyo $
  -------------------------------------------------------------------------
...
class Ticket extends CommonITILObject {
    ...
    function canAssignToMe() {
        ...
        return (Session::haveRight("steal_ticket", "1") || (Session::haveRight("own_ticket", "1") && $this->countUsers(parent::ASSIGN) == 0));
    }
    ...
    function canUpdate() {
        ...
        return (Session::haveRight('update_ticket', 1) || Session::haveRight('create_ticket', 1) || Session::haveRight('assign_ticket', 1) || Session::haveRight('own_ticket', 1) || Session::haveRight('steal_ticket', 1));
    }
    ...
    function canView() {
        ...
        return (Session::haveRight("show_all_ticket", "1") || Session::haveRight('create_ticket', '1') || Session::haveRight('update_ticket', '1') || Session::haveRight('show_all_ticket', '1') || Session::haveRight("show_assign_ticket", '1') || Session::haveRight("own_ticket", '1') || Session::haveRight('validate_ticket', '1') || Session::haveRight("show_group_ticket", '1'));
    }
    ...
    function canViewItem() {
        ...
        return (Session::haveRight("show_all_ticket", "1") || $this->fields["users_id_recipient"] === Session::getLoginUserID() || $this->isUser(parent::REQUESTER, Session::getLoginUserID()) || $this->isUser(parent::OBSERVER, Session::getLoginUserID()) || (Session::haveRight("show_group_ticket", '1') && isset($_SESSION["glpigroups"]) && ($this->haveAGroup(parent::REQUESTER, $_SESSION["glpigroups"]) || $this->haveAGroup(parent::OBSERVER, $_SESSION["glpigroups"]))) || (Session::haveRight("show_assign_ticket", '1') && ($this->isUser(parent::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::ASSIGN, $_SESSION["glpigroups"])) || (Session::haveRight('assign_ticket', 1) && $this->fields["status"] == 'new')
                )
                ) || (Session::haveRight('validate_ticket', '1') && TicketValidation::canValidate($this->fields["id"]))
                );
    }
    ...
    function canSolve() {
        ...
        return ((Session::haveRight("update_ticket", "1") || $this->isUser(parent::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::ASSIGN, $_SESSION["glpigroups"]))) && self::isAllowedStatus($this->fields['status'], 'solved'));
    }
    ...
    function canApprove() {
        ...
        return ($this->fields["users_id_recipient"] === Session::getLoginUserID() || $this->isUser(parent::REQUESTER, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::REQUESTER, $_SESSION["glpigroups"])));
    }
    ...
    function canUpdateItem() {
        ...
        if ($this->numberOfFollowups() == 0 && $this->numberOfTasks() == 0 && ($this->isUser(parent::REQUESTER, Session::getLoginUserID()) || $this->fields["users_id_recipient"] === Session::getLoginUserID())) {
            return true;
        }
    ...
    function canDeleteItem() {
        ...
        if (($this->isUser(parent::REQUESTER, Session::getLoginUserID()) || $this->fields["users_id_recipient"] === Session::getLoginUserID()) && $this->numberOfFollowups() == 0 && $this->numberOfTasks() == 0 && $this->fields["date"] == $this->fields["date_mod"]) {
            return true;
        }
    ...
    function getDefaultActor($type) {
        ...
        if ($type == self::ASSIGN) {
            if (Session::haveRight("own_ticket", "1") && $_SESSION['glpiset_default_tech']) {
            ...
            case __CLASS__ :
                switch ($tabnum) {
                    ...
                    case 2 :
                        ...
                        /* //SUPRISERVICE */
                        if (Toolbox::isMobile())
                            $item->showSolutionFormMobile($_POST['load_kb_sol']);
                        else
...
//       }
        ...
        if (is_numeric(Session::getLoginUserID(false)) && !Session::haveRight("update_ticket", "1")) {
            ...
            if (TicketValidation::canValidate($this->fields['id']) || $ticketval->canCreate() || isset($input["_rule_process"])) {
            ...
            if ($this->numberOfFollowups() == 0 && $this->numberOfTasks() == 0 && $this->isUser(parent::REQUESTER, Session::getLoginUserID())) {
        ...
        if (count($tt->mandatory)) {
            ...
            foreach ($tt->mandatory as $key => $val) {
                if ((!$check_allowed_fields_for_template || in_array($key, $allowed_fields)) && (isset($input[$key]) && (empty($input[$key]) || $input[$key] == 'NULL'))) {
        ...
        if (isset($this->input["_add_validation"]) && $this->input["_add_validation"] > 0) {
            ...
            if (!is_numeric(Session::getLoginUserID(false)) || $validation->can(-1, 'w', $values)) { // cron or allowed user
                $validation->add($values);

                Event::log($this->fields['id'], "ticket", 4, "tracking", (is_numeric(Session::getLoginUserID(false)) ? $_SESSION["glpiname"] : 'cron')
                        . "  " . $LANG['log'][21]);
            }
        }
    ...
    function pre_updateInDB() {
        ...
        if ($this->fields['takeintoaccount_delay_stat'] == 0 && (Session::haveRight("global_add_tasks", "1") || Session::haveRight("global_add_followups", "1") || ($this->isUser(parent::ASSIGN, Session::getLoginUserID())) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::ASSIGN, $_SESSION['glpigroups'])))) {
    ...
    function computeTakeIntoAccountDelayStat() {
        ...
        if (isset($this->fields['id']) && !empty($this->fields['date'])) {
            ...
            if ($calendars_id > 0 && $calendar->getFromDB($calendars_id)) {
                return max(0, $calendar->getActiveTimeBetween($this->fields['date'], $_SESSION["glpi_currenttime"]));
            }
    ...
    function post_updateItem($history = 1) {
        ...
        if (count($this->updates)) {
            ...
            if ((in_array("solutiontypes_id", $this->updates) || in_array("solution", $this->updates)) && (in_array($this->input["status"], $this->getSolvedStatusArray()) || in_array($this->input["status"], $this->getClosedStatusArray()))) { // auto close case
        ...
        $rate = EntityData::getUsedConfig('inquest_config', $this->fields['entities_id'], 'inquest_rate');
        $delay = EntityData::getUsedConfig('inquest_config', $this->fields['entities_id'], 'inquest_delay');
...
//          }
        ...
        if (!isset($input["type"])) {
            $input['type'] = EntityData::getUsedConfig('tickettype', $input['entities_id'], '', Ticket::INCIDENT_TYPE);
        }
    ...
    function post_addItem() {
        ...
        // Log this event
        Event::log($this->fields['id'], "ticket", 4, "tracking", getUserName(Session::getLoginUserID()) . " " . $LANG['log'][20]);
        ...
        if (isset($this->input["_add_validation"]) && $this->input["_add_validation"] > 0) {
            ...
            if (!is_numeric(Session::getLoginUserID(false)) || isset($this->input["_rule_process"]) || $validation->can(-1, 'w', $values)) { // cron or allowed user
                $validation->add($values);
    ...
    function updateInicioAtendimento($id) {
        global $DB;

        $inicio = date('Y-m-d H:i:s');

        $query = "UPDATE glpi_tickets
                SET inicio_atendimento = '{$inicio}'
                WHERE id = {$id}
    ...
    static function countTicketsForUser($user_id) {
        global $DB;

        $query = "	SELECT * 
						FROM glpi_tickets t, glpi_tickets_users tu
						WHERE t.id = tu.tickets_id
						AND tu.`type` = 2
						AND tu.users_id = {$user_id}
						AND t.`status` NOT IN ('solved','closed')";
        $rs_query = $DB->query($query);

        return $DB->numrows($rs_query);
    }

    function checkWarrantyForItem($itemtype, $items_id) {
        global $DB;
        ...
        foreach ($DB->request($query) as $data) {
            if (isset($data['warranty_date'])) {
                $warranty_start = strtotime($data['warranty_date']);
                $months = $data['warranty_duration'];
                $expiration = strtotime($data['warranty_date'] . "+{$months} months");
    ...
    function checkContractTimeForItem($items_id) {
        global $DB;

        $query = "SELECT * 
                FROM supridesk_contracts_items_printers cip,
                    supridesk_contracts_items ci,
                    glpi_contracts c
                WHERE cip.contracts_items_id = ci.id
                AND ci.contracts_id = c.id
                AND cip.printers_id
                AND cip.printers_id = '$items_id'
                AND cip.is_active = 1";

        foreach ($DB->request($query) as $data) {
            if (isset($data['begin_date'])) {
                $months_to_alert = $data['duration'] - $data['notice'];
                $alert_date = strtotime($data['begin_date'] . " 00:00:00" . "+{$months_to_alert} months");
                $expiration_date = strtotime($data['begin_date'] . " 23:59:59" . "+{$data['duration']} months");

                /*
                  $alert_date = $this->add_date($data['begin_date'], 0, $data['duration'] - $data['notice'],0);
                  print "<br>inicio: ".$data['begin_date'];
                  print "<br>duracao: ".$data['duration'];
                  print "<br>meses antes pra alertar: ".$data['notice'];
                  print "<br>inicio alerta: ".$alert_date;
                 */

                $msg = "";
                if (strtotime("now") > $expiration_date)
                    $msg = "O contrato referente a este equipamento está expirado desde " . date('d-m-Y', $expiration_date);
                else if (strtotime("now") > $alert_date)
                    $msg = "O contrato referente a este equipamento expirará em " . date('d-m-Y', $expiration_date);

                if (!empty($msg))
                    return "<div class='red' style='font-size: 12px; font-weight: bold;'>" . $msg . ".</div><br>";
            }
        }
    ...
    function addCartridgeItem($tickets_id, $cartridgeitems_id, $is_request = 0) {
        global $DB;

        //atualiza os registros atuais para deletados
        $query = "UPDATE supridesk_tickets_cartridgeitems
						SET is_deleted = 1
        ...
        $is_deleted = $cartridgeitems_id > 0 ? 0 : 1;
        $query2 = "INSERT INTO supridesk_tickets_cartridgeitems (
						tickets_id,
						cartridgeitems_id,
						date,
						is_request,
    ...
    function getActiveCartridgeItem($tickets_id, $is_request = 0) {
        global $DB;

        //atualiza os registros atuais para deletados
        $query = "	SELECT CONCAT(name, ' - ', ref) as consumivel
						FROM glpi_cartridgeitems ci, supridesk_tickets_cartridgeitems tci
						WHERE tci.tickets_id = $tickets_id
							AND tci.cartridgeitems_id = ci.id
							AND tci.is_request = $is_request
							AND tci.is_deleted = 0";
        $rs_query = $DB->query($query);

        if ($DB->numrows($rs_query) > 0) {
            $row = $DB->fetch_array($rs_query);
            return $row['consumivel'];
        }
        return null;
    }
    ...
    function countActiveTicketsForItem($itemtype, $items_id) {
        ...
        return countElementsInTable($this->getTable(), "`" . $this->getTable() . "`.`itemtype` = '$itemtype'
                                    AND `" . $this->getTable() . "`.`items_id` = '$items_id'
                                    AND `" . $this->getTable() . "`.`status`
                                       NOT IN ('" . implode("', '", array_merge($this->getSolvedStatusArray(), $this->getClosedStatusArray())
                ) . "')");
    ...
    function countSolvedTicketsForItemLastDays($itemtype, $items_id, $days) {
        ...
        return countElementsInTable($this->getTable(), "`" . $this->getTable() . "`.`itemtype` = '$itemtype'
                                    ...
                                    AND `" . $this->getTable() . "`.`status`
                                          IN ('" . implode("', '", array_merge($this->getSolvedStatusArray(), $this->getClosedStatusArray())
                ) . "')");
    }
    ...
    function updateDateMod($ID, $no_stat_computation = false, $users_id_lastupdater = 0) {
        ...
        if ($this->getFromDB($ID)) {
            if (!$no_stat_computation && (Session::haveRight("global_add_tasks", "1") || Session::haveRight("global_add_followups", "1") || ($this->isUser(parent::ASSIGN, Session::getLoginUserID())) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::ASSIGN, $_SESSION['glpigroups'])))) {
    ...
    function canAddFollowups() {
        ...
        return ((Session::haveRight("add_followups", "1") && ($this->isUser(parent::REQUESTER, Session::getLoginUserID()) || $this->fields["users_id_recipient"] === Session::getLoginUserID())) || Session::haveRight("global_add_followups", "1") || (Session::haveRight("group_add_followups", "1") && isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::REQUESTER, $_SESSION['glpigroups'])) || ($this->isUser(parent::ASSIGN, Session::getLoginUserID())) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(parent::ASSIGN, $_SESSION['glpigroups'])));
    }
    ...
    function getSearchOptions() {
        ...
        /* //SUPRISERVICE */
        $tab[15]['css_class'] = 'datetimepicker';
        ...
        /* //SUPRISERVICE */
        $tab[131]['mobile_readonly'] = true;

        $tab[200]['table'] = $this->getTable();
        $tab[200]['field'] = 'inicio_atendimento';
        $tab[200]['name'] = $LANG['custom_chamado'][18];
        $tab[200]['nosearch'] = true;
        $tab[200]['nosort'] = true;
        $tab[200]['massiveaction'] = false;
        $tab[200]['datatype'] = 'datetime';
        $tab[200]['css_class'] = 'datetimepicker';
        //--
        ...
        /* //SUPRISERVICE */
        $tab[191]['table'] = 'glpi_ticketfollowups';
        $tab[191]['field'] = 'date';
        $tab[191]['name'] = $LANG['job'][9] . " - " . "Data";
        $tab[191]['datatype'] = 'datetime';
        $tab[191]['forcegroupby'] = true;
        $tab[191]['splititems'] = true;
        $tab[191]['massiveaction'] = false;
        $tab[191]['joinparams'] = array('jointype' => 'child',
            'condition' => $followup_condition);
    ...
    static function getAllStatusArray($withmetaforsearch = false) {
        ...
        $tab = array('new' => $LANG['joblist'][9],
            ...
            'atendimento' => $LANG['custom_chamado'][20],
            'waiting' => $LANG['joblist'][26], // SLA Stop
            'waiting1' => 'Pendente Fornecedor', // SLA Stop
            'waiting4' => 'Pendente Supriservice',
            'solved' => $LANG['joblist'][32],
            'closed' => $LANG['joblist'][33],
            'approbation' => 'Aguardando Aprovação (Cliente)', // SLA Stop
            'approbation1' => 'Aguardando Aprovação (Supriservice)',
            'canceled' => 'Cancelado', // SLA Stop
        ...
        if ($withmetaforsearch) {
            ...
            $tab['oldest'] = $LANG['joblist'][32] . " + " . $LANG['joblist'][33] . " + " . "Cancelado";
    ...
    static function getCanceledStatusArray() {
                    ...
                    foreach ($CFG_GLPI["linkgroup_types"] as $itemtype) {
                        if (($item = getItemForItemtype($itemtype)) && parent::isPossibleToAssignType($itemtype)) {
                            ...
                            $query = "SELECT *
                               FROM `$itemtable`
                               WHERE ($group_where) " .
                                    getEntitiesRestrictRequest("AND", $itemtable, "", $entity_restrict, $item->maybeRecursive());
    ...
    static function dropdownAllDevices($myname, $itemtype, $items_id = 0, $admin = 0, $users_id = 0, $entity_restrict = -1, $options = array()) {
        ...
        if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] == 0) {
            ...
            if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] & pow(2, HELPDESK_ALL_HARDWARE)) {
                ...
                if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] & pow(2, HELPDESK_MY_HARDWARE)) {
                    /* //SUPRISERVICE */
                    if (isset($options["dropdown_label"]))
                        echo $options["dropdown_label"];
                    else
                ...
                //var_export($types);
                $type_add = array("Transport" => "Veículos");
                $types = array_merge($types, $type_add);

                $type_add2 = array("ConsumableItem" => "Insumos");
                $types = array_merge($types, $type_add2);

                $type_add3 = array("Document" => "Documentos");
                $types = array_merge($types, $type_add3);
                
                $type_add4 = array("Stock" => "Peças");
                $types = array_merge($types, $type_add4);
                ...
                if (array_key_exists("myname", $options))
                    $params['myname'] = $options["myname"];
                else
                    $params['myname'] = "items_id";
    ...
    static function dropdownAllModelDevices($myname, $itemtype, $items_id = 0, $admin = 0, $users_id = 0, $entity_restrict = -1, $options = array()) {
        global $LANG, $CFG_GLPI, $DB;
        ...
        if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] == 0) {
            ...
            if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] & pow(2, HELPDESK_ALL_HARDWARE)) {
                ...
                $types = array("ComputerModel" => "Computador", "MonitorModel" => "Monitor", "PrinterModel" => "Impressora");
                ...
                //echo "<option value='Doc'>Documentos</option>\n";                
                echo "</select>";
    ...
    static function dropdownAllTypeDevices($myname, $itemtype, $items_id = 0, $admin = 0, $users_id = 0, $entity_restrict = -1, $options = array()) {
        global $LANG, $CFG_GLPI, $DB;
        ...
        if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] == 0) {
            ...
            if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] & pow(2, HELPDESK_ALL_HARDWARE)) {
                ...
                //var_export($itemtype);
                $types = array("ComputerType" => "Computador", "MonitorType" => "Monitor", "PrinterType" => "Impressora");
    ...
    function showCost() {
        ...
        $this->fields["cost_time"] = $atendimento->custoHoras($ID);
        ...
        echo "<td class='b'>" . $atendimento->horasTotal($ID) . "</td>";
        echo "</tr>";
        ...
        /*  if ($canedit) {
          echo "<input type='text' maxlength='100' size='15' name='cost_time' value='" .
          Html::formatNumber($atendimento->custoHoras($ID), true) . "'>";
          // Html::formatNumber($this->fields["cost_time"]);
          } else {
          echo Html::formatNumber($atendimento->custoHoras($ID));
          // echo Html::formatNumber($this->fields["cost_time"]);
          } */
        echo "</td></tr>\n";
        ...
        if ($canedit) {
            echo "<input type='text' readonly = 'readonly' maxlength='100' size='15' name='cost_material' value='" .
        ...
        echo "<td class='b'>";
        // echo self::trackingTotalCost($this->fields["actiontime"], $this->fields["cost_time"], $this->fields["cost_fixed"], $this->fields["cost_material"], false);
        echo self::trackingTotalCost($this->fields["cost_time"], $this->fields["cost_fixed"], $this->fields["cost_material"], false);
        echo "</td></tr>\n";
        ...
        //******* SUPRISERVICE *****//
        echo "<form method='post' action=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.form.php?id=" . $ID . "\">";
        echo "<table class='tab_cadre_fixe'>";
        echo "<tr><td class='center tab_bg_2'>";
        echo "<input type='hidden' name='id' value='" . $ID . "'>\n";
        echo "<input type='hidden' name='update_cost' value='update_cost'>\n";

        echo "<span class='small_space'>";

        $rand = mt_rand();

        $opt = array('value' => $this->fields["stocks_id"],
            'rand' => $rand,
            //'condition'=> 'quantidade > 0',
            'name' => 'dropdown_stock'
        );

        array(Dropdown::show('Stock', $opt));
        echo "</span>&nbsp;";
        echo "<span class='small_space'>";
        Dropdown::showInteger('to_add', 1, 1, 100);
        echo "</span>";
        echo $LANG['stock'][4] . "&nbsp;";
        echo "<input type='submit' name='update' value=\"" . $LANG['buttons'][8] . "\"
           class='submit'></td></tr>";
        echo "</table>";
        Html::closeForm();

        $query = "SELECT `supridesk_pecas_ticket`.`peca_id` AS stck_id,
                        `supridesk_pecas_ticket`.`id` AS id,
                        `supridesk_pecas_ticket`.`quantidade` AS quantidade,
                        `supridesk_pecas`.`name` AS name,
                        `supridesk_pecas`.`value` AS value                        
                FROM `supridesk_pecas_ticket`
                INNER JOIN `supridesk_pecas` ON (`supridesk_pecas`.`id`= `supridesk_pecas_ticket`.`peca_id`)
                WHERE `supridesk_pecas_ticket`.`tickets_id` = '" . $ID . "'";
        ...
        if ($DB->numrows($result) > 0) {
            echo "</th></tr>";
            echo "<tr><th>" . $LANG['stock'][2] . "</th>";
            echo "<th>" . $LANG['common'][16] . "</th>";
            echo "<th>&nbsp;</th></tr>";

            while ($i < $number) {

                $quantidade = $DB->result($result, $i, "quantidade");
                $name = $DB->result($result, $i, "name");
                $value = $DB->result($result, $i, "value");
                $id_stock = $DB->result($result, $i, "stck_id");
                $id = $DB->result($result, $i, "id");

                echo "<tr class='tab_bg_1'><td class='center'>";
                echo $name . "</td>";
                echo "<td class='center'>" . $quantidade . "</td>";
                echo "<td class='tab_bg_2 center'>";
                echo "<a href='" . $CFG_GLPI["root_doc"] .
                "/front/ticket.form.php?deletesupplier=1&amp;id=$ID&amp;qtde=$quantidade&amp;vlr=$value&amp;idstk=$id_stock&amp;id_stck_tick=$id'><img src='" . $CFG_GLPI["root_doc"] . "/pics/delete.png' alt='" .
                $LANG['buttons'][6] . "'></a>";
                echo "</td></tr>";
                $i++;
            }
    ...
    static function computeTco(CommonDBTM $item) {
        ...
        if ($DB->numrows($result)) {
            while ($data = $DB->fetch_array($result)) {
                $totalcost += self::trackingTotalCost($data["actiontime"], $data["cost_time"], $data["cost_fixed"], $data["cost_material"]);
            }
        }
        return $totalcost;
    }
     ...
     * */
    // SUPRISERVICE - RETIRADO $actiontime
    static function trackingTotalCost($cost_time, $cost_fixed, $cost_material, $edit = true) {
        //return Html::formatNumber(($actiontime * $cost_time / HOUR_TIMESTAMP) + $cost_fixed + $cost_material, $edit);
        return Html::formatNumber($cost_time + $cost_fixed + $cost_material, $edit);
    }
    ...
    static function showFormHelpdesk($ID, $ticket_template = false) {
        ...
        $default_values = array('_users_id_requester_notif' => array('use_notification' => ($email == "" ? 0 : 1),
            ...
            'slas_id' => 0,
            '_add_validation' => 0,
            'type' => EntityData::getUsedConfig('tickettype', $_SESSION['glpiactive_entity'], '', Ticket::INCIDENT_TYPE),
            '_right' => "id");
        ...
        if (isset($tt->predefined) && count($tt->predefined)) {
            foreach ($tt->predefined as $predeffield => $predefvalue) {
                if (isset($options[$predeffield]) && isset($default_values[$predeffield])) {
                    ...
                    if ($options[$predeffield] == $default_values[$predeffield] || (isset($options['_predefined_fields'][$predeffield]) && $options[$predeffield] == $options['_predefined_fields'][$predeffield]) || (isset($options['_tickettemplates_id']) && $options['_tickettemplates_id'] != $tt->getID())) {
        ...
        if ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"] != 0) {
            if (!$tt->isHiddenField('itemtype')) {
                ...
                echo "<td>";
                self::dropdownMyDevices($options['_users_id_requester'], $_SESSION["glpiactive_entity"], $options['itemtype'], $options['items_id']);
                self::dropdownAllDevices("itemtype", $options['itemtype'], $options['items_id'], 0, $options['_users_id_requester'], $_SESSION["glpiactive_entity"]);
    ...
    function showForm($ID, $options = array()) {
        ...
        if ($values['type'] <= 0) {
            $values['type'] = EntityData::getUsedConfig('tickettype', $values['entities_id'], '', Ticket::INCIDENT_TYPE);
        }
        ...
        $canupdate_descr = $canupdate || ($this->fields['status'] == 'new' && $this->isUser(parent::REQUESTER, Session::getLoginUserID()) && $this->numberOfFollowups() == 0 && $this->numberOfTasks() == 0);
        ...
        /* //SUPRISERVICE // OSWALDO - Implementacao da exibicao do endereco (Tela) */
        $entitydata = new EntityData();
        if ($entitydata->getFromDB($this->fields['entities_id'])) {
            echo "<br>";
            echo $entitydata->getfield('address') . ' - ' . $entitydata->getfield('town') . ' ' . $entitydata->getfield('state') . ' - CEP: ' . $entitydata->getfield('postcode');
        }
        echo "</th></tr>";
            ...
            if ($this->fields["slas_id"] > 0) {
                ...
                if ($this->fields['status'] != 'closed' && $canupdate) {
                    ...
                    echo "<a class='pointer' " . Html::addConfirmationOnAction(array($LANG['sla'][13],
                        $LANG['sla'][14]), "cleanhide('sla_action');cleandisplay('sla_choice');") .
                    ">" . $LANG['sla'][12] . '</a>';
                    echo "</span>";
        ...
        if ($canupdate || !$ID || $canupdate_descr) {
            ...
            if (($ID || $values['itilcategories_id']) && $tt->isMandatoryField("itilcategories_id")) {
                $opt['display_emptychoice'] = false;
            }
        ...
        echo "<td width='$colsize2%'>";
        echo "<input type='hidden' name='confere_status' value='" . $this->fields['solution'] . "'>";
        ...
        echo "</td>";
        echo "</tr>";
        //var_export($this->fields["id"]);
        
        if($ID){   
           
            echo "<th width='$colsize1%'>Tipo (Fechamento) &nbsp;:" . $tt->getMandatoryMark('type') . "</th>";
            echo "<td width='$colsize2%'>";
            // Permit to set type when creating ticket without update right
            if ($canupdate || !$ID) {
                $opt = array('value' => $this->fields["type_fechamento"]);                
                $rand = self::dropdownType('type_fechamento', $opt);
               
            } else {
                echo self::getTicketTypeName($this->fields["type_fechamento"]);
            }
            echo "</td>";            
        }
       ...
       /* echo "<tr class='tab_bg_1'>";
        ...
        echo "<tr class='tab_bg_1'>";
        echo "<th rowspan='2'>" . $LANG['joblist'][2] . "&nbsp;:" . $tt->getMandatoryMark('priority') .
        "</th>";
        echo "<td rowspan='2'>";
        ...
        if ($canupdate || !$ID || $canupdate_descr) {
            if ($ID) {
                ...
                if ($this->fields['itemtype'] && ($item = getItemForItemtype($this->fields['itemtype'])) && $this->fields["items_id"]) {
                    ...
                    /* //SUPRISERVICE */
                    if ($this->fields['itemtype'] == 'Printer') {
                        $consumivel = $this->getActiveCartridgeItem($ID, 1);
                        if ($consumivel != null)
                            echo "<br><br><b>Consumível a ser utilizado:</b><br> {$consumivel}";
                    }
                }
            }

            //var_export('teysad');
            //       var_export($this->fields['itemtype']);
            $dev_user_id = 0;
            ...
            } else if (isset($this->users[parent::REQUESTER]) && count($this->users[parent::REQUESTER]) == 1) {
            ...
            //die();
            //var_export(array($dev_itemtype, $dev_items_id, $dev_user_id, $this->fields["entities_id"]));
            self::dropdownAllDevices("itemtype", $dev_itemtype, $dev_items_id, 1, $dev_user_id, $this->fields["entities_id"]);

            /* //SUPRISERVICE - Ao abrir um chamado pela aba de chamados da impressora, carrega o combo para informar o suprimento */
            if (!$ID && $dev_itemtype == 'Printer') {
                $printer = new Printer();
                $printer->getFromDB($dev_items_id);
                echo "<br><br><b>Selecione o consumível a ser utilizado:</b><br>";
                CartridgeItem::dropdownForTicket($printer, true);

                /* //SUPRISERVICE */
                $expiracao = $this->checkContractTimeForItem($_REQUEST['items_id']);
                if ($expiracao !== true)
                    echo $expiracao;
            }
            //--
        ...
        $datetime = NULL;
        if ($ID && $result = $DB->query($query)) {
            $row = $DB->fetch_row($result);
            $printerLogs->getFromDB(current($row));
            $datetime = $printerLogs->fields["date"];
        }


        if ($this->fields['itemtype'] == "Transport") {

            $query_km = "SELECT *
					FROM `supridesk_veiculos_km`
					WHERE `tickets_id` = {$this->fields['id']}
					ORDER BY printerlogkm_date DESC LIMIT 1";

            $id_km_hid = 0;

            if ($ID && $result = $DB->query($query_km)) {
                $rowkm = $DB->fetch_row($result);

                $trans = new TransportLogs();
                $trans->getFromDB(current($rowkm));
                $datetimelog = $trans->fields["printerlogkm_date"];
            ...
            echo "<input type='hidden' name='id_km_hid' value='" . $id_km_hid . "'>\n";
            echo "<table class='tab_cadre_fixe'>";
            echo "<tr class='tab_bg_1'>";
            echo "<th colspan='6'>Kilometragem do Veículo</th>";
            echo "</tr>";

            echo "<tr class='tab_bg_1'>";
            echo "<th width='25%'>Kilometragem :</th>";
            echo "<td><input type='text' name='km' value='" . $kmlog . "' size='20'>";
            echo "</td>";

            echo "<th width='25%'>Data :</th>";
            echo "<td>";
            if ($canupdate) {
                Html::showDateTimeFormItem("printerlogkm_date", $datelog, 1, true);
            } else {
                echo Html::convDateTime($datelog);
            }
            echo "</td>";

            echo "</tr>";
            echo "</table>";
        } elseif ($this->fields['itemtype'] == "Printer" || $this->fields['itemtype'] == "printer") {


            echo "<table class='tab_cadre_fixe'>";
            echo "<tr class='tab_bg_1'>";
            echo "<th colspan='6'>Contadores do equipamento relacionado</th>";
            echo "</tr>";

            echo "<tr class='tab_bg_1'>";
            echo "<th width='20%'>Mono :</th>";
            echo "<td>";
            echo Html::autocompletionTextField($printerLogs, 'pages_n_b', array('size' => '20'));
            echo "</td>";
            echo "<th width='20%'>Color :</th>";
            echo "<td>";
            echo Html::autocompletionTextField($printerLogs, 'pages_color', array('size' => '20'));
            echo "</td>";

            echo "<th width='20%'>Data do contador :</th>";
            echo "<td>";
            if ($canupdate) {
                Html::showDateTimeFormItem("printerlogs_date", $datetime, 1, true);
            } else {
                echo Html::convDateTime($datetime);
            }
            echo "</td>";
            echo "</tr>";
            echo "</table>";
        ...
        /* //SUPRISERVICE */
        echo "<tr class='tab_bg_1'>";
        echo "<th width='$colsize1%' colspan='2'>" . $LANG['custom_chamado'][1] . "&nbsp;:</th>";
        echo "<td width='$colsize2%' colspan='3'>";
        Html::autocompletionTextField($this, 'numero_chamado_terceiro');
        echo "</td>";
        echo "</tr>";

        echo "<tr class='tab_bg_1'>";
        echo "<th width='$colsize1%' colspan='2'>" . $LANG['custom_chamado'][18] . "&nbsp;:</th>";
        echo "<td width='$colsize2%' colspan='3'>";

        /* //SUPRISERVICE */
        $inicio_atendimento = $this->fields["inicio_atendimento"];

        /* if ($inicio_atendimento == 'NULL')
          $inicio_atendimento = null;
          if ($canupdate) { */
        if ($inicio_atendimento != NULL) {
            echo Html::convDateTime($inicio_atendimento);
        } else {
            echo "<input type='submit' class='submit' name='inicio_atd' value='Iniciar Primeiro Atendimento'>";
        }
        /* Html::showDateTimeFormItem("inicio_atendimento", $inicio_atendimento, 1, true);
          } else {
          echo Html::convDateTime($inicio_atendimento);
          } */
        echo "</td>";
        ...
        if ((!$ID || $canupdate || $canupdate_descr || Session::haveRight("assign_ticket", "1") || Session::haveRight("steal_ticket", "1")) && !isset($options['template_preview'])) {
        ...
        echo "</table>";
        echo "<input type='hidden' name='id' value='$ID'>";
        echo "<input type='hidden' name='typeitem' value='{$this->fields['itemtype']}'>";
        echo "<input type='hidden' name='itemsid' value='{$this->fields['items_id']}'>";
        echo "<input type='hidden' name='itilcategories_id_' value='{$this->fields['itilcategories_id']}'>";
    ...
    static function showCentralList($start, $status = "process", $showgrouptickets = true) {
        ...
        switch ($status) {
            /* //SUPRISERVICE */
            case "atendimento" : // on affiche les tickets en attente
                $query .= "WHERE $is_deleted AND ($search_assign)
                             AND `status` = 'atendimento' " .
                        getEntitiesRestrictRequest("AND", "glpi_tickets");
                break;

            case "waiting1" : // on affiche les tickets en attente
                $query .= "WHERE $is_deleted AND ($search_assign)
                             AND `status` = 'waiting1' " .
                        getEntitiesRestrictRequest("AND", "glpi_tickets");
                break;

            case "waiting4" : // on affiche les tickets en attente
                $query .= "WHERE $is_deleted AND ($search_assign)
                             AND `status` = 'waiting4' " .
                        getEntitiesRestrictRequest("AND", "glpi_tickets");
                break;

            case "approbation" : // on affiche les tickets en attente
                $query .= "WHERE $is_deleted AND ($search_assign)
                             AND `status` = 'approbation' " .
                        getEntitiesRestrictRequest("AND", "glpi_tickets");
                break;

            case "approbation1" : // on affiche les tickets en attente
                $query .= "WHERE $is_deleted AND ($search_assign)
                             AND `status` = 'approbation1' " .
                        getEntitiesRestrictRequest("AND", "glpi_tickets");
                break;
            //--
            ...
            default :
                ...
                $query .= "WHERE $is_deleted AND ($search_users_id)
                            AND (`status` IN ('new', 'plan', 'assign', 'waiting', 'atendimento'))
        ...
        if ($numrows > 0) {
            ...
            if ($showgrouptickets) {
                switch ($status) {
                    ...
                    case "waiting" :
                        ...
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle($LANG['joblist'][13] . " (" . $LANG['joblist'][26] . ")", $number, $numrows) . "</a>";
                        break;
                    /* //SUPRISERVICE */
                    case "waiting1" :
                        foreach ($_SESSION['glpigroups'] as $gID) {
                    ...
                    case "waiting4" :
                        foreach ($_SESSION['glpigroups'] as $gID) {
                            ...
                            $options['field'][$num] = 12; // status
                            $options['searchtype'][$num] = 'equals';
                            $options['contains'][$num] = 'waiting4';
                            $options['link'][$num] = 'AND';
                            $num++;
                        }
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle($LANG['joblist'][13] . " (Pendente Supriservice)", $number, $numrows) . "</a>";
                        break;
                    ...
                    case "approbation" :
                        foreach ($_SESSION['glpigroups'] as $gID) {
                            ...
                            $options['field'][$num] = 12; // status
                            $options['searchtype'][$num] = 'equals';
                            $options['contains'][$num] = 'approbation';
                            $options['link'][$num] = 'AND';
                            $num++;
                        }
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle("Aguardando Aprovação (Cliente)" . " (" . $LANG['joblist'][26] . ")", $number, $numrows) . "</a>";
                        break;
                    ...
                    case "approbation1" :
                        foreach ($_SESSION['glpigroups'] as $gID) {
                            ...
                            $options['field'][$num] = 12; // status
                            $options['searchtype'][$num] = 'equals';
                            $options['contains'][$num] = 'approbation1';
                            $options['link'][$num] = 'AND';
                            $num++;
                        }
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle("Aguardando Aprovação (Supriservice)" . " (" . $LANG['joblist'][26] . ")", $number, $numrows) . "</a>";
                        break;
                    //--
                    case "process" :
                        foreach ($_SESSION['glpigroups'] as $gID) {
                ...
                switch ($status) {
                    ...
                    case "atendimento" :
                        ...
                        $options['contains'][0] = 'atendimento';
                        $options['link'][0] = 'AND';
                        ...
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle($LANG['supriservice'][5], $number, $numrows) . "</a>";
                        break;

                    case "waiting1" :
                        $options['field'][0] = 12; // status
                        $options['searchtype'][0] = 'equals';
                        $options['contains'][0] = 'waiting1';
                        $options['link'][0] = 'AND';
                        ...
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle($LANG['joblist'][13] . " (Pendente Fornecedor)", $number, $numrows) . "</a>";
                        break;

                    case "waiting4" :
                        $options['field'][0] = 12; // status
                        $options['searchtype'][0] = 'equals';
                        $options['contains'][0] = 'waiting4';
                        $options['link'][0] = 'AND';
                    ...
                    case "approbation" :
                        $options['field'][0] = 12; // status
                        $options['searchtype'][0] = 'equals';
                        $options['contains'][0] = 'approbation';
                        $options['link'][0] = 'AND';
                        ...
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle("Aguardando Aprovação (Cliente)", $number, $numrows) . "</a>";
                        break;

                    case "approbation1" :
                        $options['field'][0] = 12; // status
                        $options['searchtype'][0] = 'equals';
                        $options['contains'][0] = 'approbation1';
                        $options['link'][0] = 'AND';
                        ...
                        echo "<a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
                        Toolbox::append_params($options, '&amp;') . "\">" .
                        Html::makeTitle("Aguardando Aprovação (Supriservice)", $number, $numrows) . "</a>";
                        break;
                    //--
                    case "waiting" :
                        $options['field'][0] = 12; // status
            ...
            switch ($status) {
                /* //SUPRISERVICE */
                case 'atendimento' :
                    echo $LANG['supriservice'][5];
                    break;

                case 'waiting1' :
                    echo $LANG['joblist'][13] . " (Pendente Fornecedor)";
                    break;

                case 'waiting4' :
                    echo $LANG['joblist'][13] . " (Pendente Supriservice)";
                    break;

                case 'approbation' :
                    echo "Aguardando Aprovação (Cliente)";
                    break;

                case 'approbation1' :
                    echo "Aguardando Aprovação (Supriservice)";
                    break;
                //--
                case 'waiting' :
    ...
    static function showCentralCount($foruser = false) {
        ...
        if ($foruser) {
            ...
            if (Session::haveRight("show_group_ticket", '1') && isset($_SESSION["glpigroups"]) && count($_SESSION["glpigroups"])) {
        ...
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
        Toolbox::append_params($options, '&amp;') . "\">" . $LANG['custom_chamado'][20] . "</a></td>";
        echo "<td>" . $status["atendimento"] . "</td></tr>";
        //--
        $options['contains'][0] = 'waiting';
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
        ...
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
        Toolbox::append_params($options, '&amp;') . "\">Pendente Supriservice</a></td>";
        echo "<td>" . $status["waiting1"] . "</td></tr>";

        $options['contains'][0] = 'approbation';
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
        Toolbox::append_params($options, '&amp;') . "\">Aguardando Aprovação (Cliente)</a></td>";
        echo "<td>" . $status["approbation"] . "</td></tr>";

        $options['contains'][0] = 'approbation1';
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
        Toolbox::append_params($options, '&amp;') . "\">Aguardando Aprovação (Supriservice)</a></td>";
        echo "<td>" . $status["approbation1"] . "</td></tr>";
        //--
        $options['contains'][0] = 'solved';
        echo "<tr class='tab_bg_2'>";
        echo "<td><a href=\"" . $CFG_GLPI["root_doc"] . "/front/ticket.php?" .
    ...
    static function showListForItem(CommonDBTM $item) {
        ...
        array_push($_SESSION['glpiactiveprofile']['helpdesk_item_type'], 'ConsumableItem', 'Transport');
    ...
    static function showShort($id, $followups, $output_type = HTML_OUTPUT, $row_num = 0, $id_for_massaction = 0) {
        ...
        if ($job->getFromDB($id)) {
            ...
            if (($candelete || $canupdate) && $output_type == HTML_OUTPUT && $id_for_massaction) {
            ...
            if ($job->fields['status'] == 'closed') {
            ...
            } else if ($job->fields['begin_waiting1_date']) { /* //SUPRISERVICE */
                $second_col = $LANG['joblist'][15];
                if ($output_type == HTML_OUTPUT) {
                    $second_col .= "&nbsp;:<br>";
                } else {
                ...
                $second_col .= Html::convDateTime($job->fields['begin_waiting1_date']);
            } else if ($job->fields['begin_approbation_date']) { /* //SUPRISERVICE */
                $second_col = $LANG['joblist'][15];
                if ($output_type == HTML_OUTPUT) {
                    $second_col .= "&nbsp;:<br>";
                } else {
                ...
                $second_col .= Html::convDateTime($job->fields['begin_approbation_date']);
            } else if ($job->fields['begin_canceled_date']) { /* //SUPRISERVICE */
                $second_col = $LANG['joblist'][15];
                if ($output_type == HTML_OUTPUT) {
                    $second_col .= "&nbsp;:<br>";
                } else {
                ...
                $second_col .= Html::convDateTime($job->fields['begin_canceled_date']);
            ...
            // Third Column
            echo Search::showItem($output_type, "<span class='b'>" . parent::getPriorityName($job->fields["priority"]) .
                    "</span>", $item_num, $row_num, "$align bgcolor='$bgcolor'");
            ...
            if (isset($job->users[parent::REQUESTER]) && count($job->users[parent::REQUESTER])) {
                foreach ($job->users[parent::REQUESTER] as $d) {
                    ...
                    $fourth_col .= Html::showToolTip($userdata["comment"], array('link' => $userdata["link"],
                                'display' => false));
                    $fourth_col .= "<br>";
                }
            }
            ...
            if (isset($job->users[parent::ASSIGN]) && count($job->users[parent::ASSIGN])) {
                foreach ($job->users[parent::ASSIGN] as $d) {
                    ...
                    $fifth_col .= Html::showToolTip($userdata["comment"], array('link' => $userdata["link"],
                                'display' => false));
                    $fifth_col .= "<br>";
                }
            }
            ...
            // Seventh column
            echo Search::showItem($output_type, "<span class='b'>" .
                    Dropdown::getDropdownName('glpi_itilcategories', $job->fields["itilcategories_id"]) .
                    "</span>", $item_num, $row_num, $align);
            ...
            if ($output_type == HTML_OUTPUT) {
                $eigth_column .= "&nbsp;" . Html::showToolTip($job->fields['content'], array('display' => false,
                            'applyto' => "ticket" .
                            $job->fields["id"] . $rand));
            }
    ...
    static function showVeryShort($ID, $forcetab = '') {
        ...
        if ($job->getFromDBwithData($ID, 0)) {
            ...
            ")&nbsp;";
            Html::showToolTip($job->fields['content'], array('applyto' => 'ticket' . $job->fields["id"] . $rand));
    ...
    static function cronAlertNotClosed($task) {
        ...
        foreach (Entity::getEntitiesToNotify('notclosed_delay') as $entity => $value) {
            ...
            if (!empty($tickets)) {
                if (NotificationEvent::raiseEvent('alertnotclosed', new self(), array('items' => $tickets,
                            'entities_id' => $entity))) {
...
//                }
    ...
    static function getEntityIDFromEquipamento($table, $value) {
        global $DB;

        $query = " SELECT id, entities_id FROM $table WHERE serial = '$value' and is_deleted = 0 ";
        $result = $DB->query($query);
    ...
    static function getUltimoAnalistaID($items_id, $coordenador_id) {
        global $DB;

        $query = "select users_id 
                from glpi_tickets t, glpi_tickets_users tu, glpi_users u
                where t.id = tu.tickets_id
                and tu.users_id = u.id
                and items_id = {$items_id}
                and tu.type = 2
                and u.usercategories_id in (1,4)
                order by date desc
                limit 1 ";
        $result = $DB->query($query);
        ...
        if ($DB->numrows($result) > 0) {
            ...
            return $line['users_id'];
        }

        //ID do Coordenador Tecnico
        return 41;
    }

    static function jaPossuiChamadoMes($itemtype, $items_id) {
        global $DB;

        //and itilcategories_id in (4,5,11)
        $query = "select *
                from glpi_tickets t
                where is_deleted = 0
                and
                (
                        ( status in ('new', 'assign', 'waiting') and month(t.date) = month(CURRENT_DATE()) and year(t.date) = year(CURRENT_DATE()) )
                )
    ...
    static function jaPossuiSolucaoMes($itemtype, $items_id) {
        global $DB;
        ...
        $query = "select *
                ...
                and
                (
                        ( status in ('solved', 'closed') and month(t.solvedate) = month(CURRENT_DATE()) and year(t.solvedate) = year(CURRENT_DATE()) )
                )
                and itemtype = '{$itemtype}'
                and items_id = {$items_id} ";
        $result = $DB->query($query);
    ...
    //******* SUPRISERVICE *****//
    static function updateCostMaterial($ID, $valor) {

        global $DB;

        $query = "SELECT `cost_material` FROM `glpi_tickets` WHERE id ='" . $ID . "'";
        $result = $DB->query($query);
        $line = $DB->fetch_array($result);

        $valor_total = $line['cost_material'] - $valor;

        $query = "UPDATE `glpi_tickets` SET `cost_material` = '" . $valor_total . "' WHERE `id` = '" . $ID . "'";
        $result = $DB->query($query);
    }

    //******* SUPRISERVICE *****//
    static function returnStock($ID, $qtde) {

        global $DB;

        $query = "SELECT `quantidade` FROM `supridesk_pecas` WHERE id = '" . $ID . "'";
        $result = $DB->query($query);
        $line = $DB->fetch_array($result);

        $quantidade_total = $line['quantidade'] + $qtde;

        $query = "UPDATE `supridesk_pecas` SET `quantidade` = '" . $quantidade_total . "' WHERE `id` = '" . $ID . "'";
        $result = $DB->query($query);
    }

    //******* SUPRISERVICE *****//
    static function delStockTicket($ID) {

        global $DB;

        $query = "DELETE FROM `supridesk_pecas_ticket` WHERE `id`= '" . $ID . "'";
        $result = $DB->query($query);
    }

    //******* SUPRISERVICE *****//
    static function buscaLocalidade($table, $id) {
        global $DB;
    ...
    //******* SUPRISERVICE *****//
    static function buscaContrato($items_id) {
        global $DB;
    ...
    //******* SUPRISERVICE *****//
    static function updateTitle($name, $id) {
        global $DB;

        $query = "UPDATE `glpi_tickets` SET `name` = '" . $name . "' WHERE `id` = {$id}";
        $result = $DB->query($query);
    }

    //******* SUPRISERVICE *****//
    static function verificaCategoria($type, $id_categoria) {
        global $DB;
        ...
        $query = "SELECT $campo FROM glpi_itilcategories WHERE id = {$id_categoria}";
        $result = $DB->query($query);

        $array = $DB->fetch_array($result);
        //var_export($array[$campo]);
        // die();

        if ($array[$campo] == 1) {
            return true;
        } elseif ($array[$campo] == 0) {
            return false;
        }
    }
