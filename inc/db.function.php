<?php
/*
 * @version $Id: db.function.php 19366 2012-10-01 14:13:13Z moyo $
 -------------------------------------------------------------------------
...
function getTableForItemType($itemtype) {
   ...
   if (isset($CFG_GLPI['glpitablesitemtype'][$itemtype])) {
      ...
      /*//SUPRISERVICE
      *   Caso a classe de $itemtype tenha a varável
      *   $supridesk_custom_table setada, ela será a tabela
      *   de retorno e não a padrão do GLPI (ex.: glpi_nome_classe_plural)
      */
      if (class_exists($itemtype)) {
         $vars = get_class_vars($itemtype);
         if (array_key_exists("supridesk_custom_table",$vars)) {
            $supridesk_custom_table = $vars['supridesk_custom_table'];

            $CFG_GLPI['glpitablesitemtype'][$itemtype]               = $supridesk_custom_table;
            $CFG_GLPI['glpiitemtypetables'][$supridesk_custom_table] = $itemtype;
            return $supridesk_custom_table;
         }
      }
