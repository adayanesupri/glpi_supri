<?php
/*
 * @version $Id: commonitilobject.class.php 18946 2012-07-19 09:40:54Z moyo $
 -------------------------------------------------------------------------
...
abstract class CommonITILObject extends CommonDBTM {
   ...
   **/
   ...
   function pre_updateInDB() {
      ...
      if ((!is_null($this->fields['begin_waiting_date']) 
              || (!is_null($this->fields['begin_waiting1_date']))
              || (!is_null($this->fields['begin_approbation_date']))
              || (!is_null($this->fields['begin_canceled_date']))
          )
          && ($key=array_search('status',$this->updates) !== false)
          && (($this->oldvalues['status'] == 'waiting' 
                  || $this->oldvalues['status'] == 'waiting1'
                  || $this->oldvalues['status'] == 'approbation'
                  || $this->oldvalues['status'] == 'canceled'
              )
         ...
         if ($calendars_id>0 && $calendar->getFromDB($calendars_id)) {
            ...
            /*//SUPRISERVICE*/
            if ($this->oldvalues['status'] == 'waiting') 
                $delay_time = $calendar->getActiveTimeBetween($this->fields['begin_waiting_date'],
                                                          $_SESSION["glpi_currenttime"]);
            elseif ($this->oldvalues['status'] == 'waiting1')
                $delay_time = $calendar->getActiveTimeBetween($this->fields['begin_waiting1_date'],
                                                          $_SESSION["glpi_currenttime"]);
            elseif ($this->oldvalues['status'] == 'approbation')
                $delay_time = $calendar->getActiveTimeBetween($this->fields['begin_approbation_date'],
                                                          $_SESSION["glpi_currenttime"]);
            elseif ($this->oldvalues['status'] == 'canceled')
                $delay_time = $calendar->getActiveTimeBetween($this->fields['begin_canceled_date'],
                                                          $_SESSION["glpi_currenttime"]);
            ...
            elseif ($this->oldvalues['status'] == 'waiting1')
                $delay_time = strtotime($_SESSION["glpi_currenttime"])
                              -strtotime($this->fields['begin_waiting1_date']);
            elseif ($this->oldvalues['status'] == 'approbation')
                $delay_time = strtotime($_SESSION["glpi_currenttime"])
                              -strtotime($this->fields['begin_approbation_date']);
            elseif ($this->oldvalues['status'] == 'canceled')
                $delay_time = strtotime($_SESSION["glpi_currenttime"])
                              -strtotime($this->fields['begin_canceled_date']);
         }
         ...
         if (isset($this->fields['slas_id']) && $this->fields['slas_id']>0) {
            ...
            if ($sla->getFromDB($this->fields['slas_id'])) {
               ...
               if ($this->oldvalues['status'] == 'waiting') {
                   ...
                   $delay_time_sla  = $sla->getActiveTimeBetween($this->fields['begin_waiting1_date'],
                                                             $_SESSION["glpi_currenttime"]);
                    $this->updates[] = "sla_waiting1_duration";
                    $this->fields["sla_waiting1_duration"] += $delay_time_sla;
                    
               } elseif ($this->oldvalues['status'] == 'approbation') {
                   $delay_time_sla  = $sla->getActiveTimeBetween($this->fields['begin_approbation_date'],
                                                             $_SESSION["glpi_currenttime"]);
                    $this->updates[] = "sla_approbation_duration";
                    $this->fields["sla_approbation_duration"] += $delay_time_sla;
                    
               } elseif ($this->oldvalues['status'] == 'canceled') {
                   $delay_time_sla  = $sla->getActiveTimeBetween($this->fields['begin_canceled_date'],
                                                             $_SESSION["glpi_currenttime"]);
                    $this->updates[] = "sla_canceled_duration";
                    $this->fields["sla_canceled_duration"] += $delay_time_sla;
               }
            }
            ...
            if ($this->oldvalues['status'] == 'waiting') {
                ...
                $this->updates[]          = "due_date";
                $this->fields['due_date'] = $sla->computeDueDate($this->fields['date'],
                                                  $this->fields["sla_waiting1_duration"]);
            } elseif ($this->oldvalues['status'] == 'approbation') {
                $this->updates[]          = "due_date";
                $this->fields['due_date'] = $sla->computeDueDate($this->fields['date'],
                                                  $this->fields["sla_approbation_duration"]);
            } elseif ($this->oldvalues['status'] == 'canceled') {
                $this->updates[]          = "due_date";
                $this->fields['due_date'] = $sla->computeDueDate($this->fields['date'],
                                                  $this->fields["sla_canceled_duration"]);
            }
         ...
         if ($this->oldvalues['status'] == 'waiting') {
            ...
            $this->updates[]                          = "waiting1_duration";
            $this->fields["waiting1_duration"] += $delay_time;

            // Reset begin_waiting1_date
            $this->updates[]                    = "begin_waiting1_date";
            $this->fields["begin_waiting1_date"] = 'NULL';
            
         } elseif ($this->oldvalues['status'] == 'approbation') {
            $this->updates[]                          = "approbation_duration";
            $this->fields["approbation_duration"] += $delay_time;

            // Reset begin_approbation_date
            $this->updates[]                    = "begin_approbation_date";
            $this->fields["begin_approbation_date"] = 'NULL';
            
         } elseif ($this->oldvalues['status'] == 'canceled') {
            $this->updates[]                          = "canceled_duration";
            $this->fields["canceled_duration"] += $delay_time;

            // Reset begin_waiting_date
            $this->updates[]                    = "begin_canceled_date";
            $this->fields["begin_canceled_date"] = 'NULL';
         }
      }
      ...
      if (($key=array_search('status',$this->updates)) !== false
         ...
         if (isset($this->fields['slas_id']) && $this->fields['slas_id']>0) {
            ...
            /*//SUPRISERVICE*/
      // Set begin waiting1 date if needed
      if (($key=array_search('status',$this->updates)) !== false
          && ($this->fields['status'] == 'waiting1'
                  || in_array($this->fields["status"], $this->getSolvedStatusArray()))) {
         $this->updates[]                    = "begin_waiting1_date";
         $this->fields["begin_waiting1_date"] = $_SESSION["glpi_currenttime"];
      ...
      // Set begin approbation date if needed
      if (($key=array_search('status',$this->updates)) !== false
          && ($this->fields['status'] == 'approbation'
                  || in_array($this->fields["status"], $this->getSolvedStatusArray()))) {
         $this->updates[]                    = "begin_approbation_date";
         $this->fields["begin_approbation_date"] = $_SESSION["glpi_currenttime"];
      ...
      // Set begin canceled date if needed
      if (($key=array_search('status',$this->updates)) !== false
          && ($this->fields['status'] == 'canceled'
                  || in_array($this->fields["status"], $this->getSolvedStatusArray()))) {
         $this->updates[]                    = "begin_canceled_date";
         $this->fields["begin_canceled_date"] = $_SESSION["glpi_currenttime"];
   ...
   function prepareInputForAdd($input) {
      ...
      if (isset($input["status"]) && $input["status"]=="waiting") {
         $input['begin_waiting_date'] = $input['date'];
            ...
            /*//SUPRISERVICE*/
      // Set begin waiting time if status is waiting
      if (isset($input["status"]) && $input["status"]=="waiting1") {
         $input['begin_waiting1_date'] = $input['date'];
      }

      if (isset($input["status"]) && $input["status"]=="approbation") {
         $input['begin_approbation_date'] = $input['date'];
      }

      if (isset($input["status"]) && $input["status"]=="canceled") {
         $input['begin_canceled_date'] = $input['date'];
      }
   ...
   **/
    ...
    * @return nothing display
   **/
   function showGroupsAssociated($type, $canedit, $showToolTip = true) {
      ...
      if (isset($this->groups[$type]) && count($this->groups[$type])) {
         foreach ($this->groups[$type] as $d) {
            ...
            if ($canedit) {
					...
					$url = $this->getFormURL();
					if ( Toolbox::isMobile() ){
						$url = str_replace("front/", "plugins/mobile/front/", $url);
					}
               echo "&nbsp;<a href='".$url."?delete_group=delete_group&amp;id=".
   ...
   **/
    ...
    * @return nothing display
   **/
   function showUsersAssociated($type, $canedit, $showToolTip = true) {
      global $CFG_GLPI, $LANG;
      ...
      if (isset($this->users[$type]) && count($this->users[$type])) {
         foreach ($this->users[$type] as $d) {
            ...
            if ($canedit) {
               ...
               echo "&nbsp;<a href='".$url."?delete_user=delete_user&amp;id=".
                     $d['id']. "&amp;".$this->getForeignKeyField()."=".$this->fields['id'].
   ...
   function showActorsPartForm($ID, $options) {
      ...
      if (!$ID) {
         ...
         if ($reqdisplay) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showUsersAssociated(self::REQUESTER, $candeleterequester, $show_tooltip);
      }
      ...
      if (!$ID) {
         ...
         if ($this->canAdminActors() && !$is_hidden['_groups_id_requester']) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showGroupsAssociated(self::REQUESTER, $candeleterequester, $show_tooltip);
      }
      echo "</td>";
      ...
      if (!$ID) {
         ...
         if ($this->canAdminActors() && !$is_hidden['_users_id_observer']) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showUsersAssociated(self::OBSERVER, $candeleteobserver, $show_tooltip);
      }
      ...
      if (!$ID) {
         ...
         if ($this->canAdminActors() && !$is_hidden['_groups_id_observer']) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showGroupsAssociated(self::OBSERVER, $candeleteobserver, $show_tooltip);
      }
      echo "</td>";
      ...
      if (!$ID) {
         ...
         if ($this->canAssign() && !$is_hidden['_users_id_assign']) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showUsersAssociated(self::ASSIGN, $candeleteassign, $show_tooltip);
      }
      ...
      if (!$ID) {
         if ($this->canAssign() && !$is_hidden['_groups_id_assign']) {
			...
			/*//SUPRISERVICE*/
			$show_tooltip = (isset($options['show_tooltip']) && $options['show_tooltip'] == false) ? false : true;
         $this->showGroupsAssociated(self::ASSIGN, $candeleteassign, $show_tooltip);
      }
...
//                        && empty($this->fields['solution']);
      ...
      if ($canedit) {
		...
		/*//SUPRISERVICE*/
		$defaultITIL = $this->fields["solution_itilcategories_id"];

		if ( $defaultITIL <= 0 )
			$defaultITIL = $this->fields["itilcategories_id"];

      if ($canedit && Session::haveRight('knowbase','w')) {
         $opt = array('value'  => $defaultITIL,
                      'entity' => $this->fields["entities_id"]);
         ...
         switch ($this->fields['type_fechamento']) {
         ...
         //var_export($opt['condition']);
        $opt['name'] = "solution_itilcategories_id";
        echo "<tr class='tab_bg_2'><td>".$LANG['custom_chamado'][2]."</td><td>";
			/*//SUPRISERVICE*/
        $toupdate = array();
        $moreparams = array('value' => '__VALUE__',
                            'rand_text' => $rand_text,
                            'type_id' => 'solutiontypes_id'.$rand_type );

        $toupdate[] = array('value_fieldname' => 'value',												  
                            'to_update'  => "show_solutiontypes_by_solutioncategory",
                            'url'        => $CFG_GLPI["root_doc"]."/ajax/dropdownSolutionTypes.php",
                            'moreparams' => $moreparams);

	$moreparams['value'] = -1;
	$toupdate[] = array('value_fieldname' => 'value',
                            'to_update'  => "show_solutiontemplates_by_solutiontypes",
                            'url'        => $CFG_GLPI["root_doc"]."/ajax/dropdownSolutionTemplate.php",
                            'moreparams' => $moreparams);
         ...
         echo Dropdown::getDropdownName("glpi_itilcategories", $defaultITIL);
      }
      ...
      if ($canedit) {
			/*//SUPRISERVICE*/
         echo "<span id='show_solutiontypes_by_solutioncategory'>";
			/*
         Dropdown::show('SolutionType',
			...
			*/
			...
			$itilcategory->getFromDB($defaultITIL);
			SolutionType::dropdownByITILCategory($itilcategory, 'solutiontypes_id'.$rand_type, $this->getField('solutiontypes_id'));

			$opt = array('value' => '__VALUE__',
							'rand_text' => $rand_text,
							'type_id' => 'solutiontypes_id'.$rand_type );

			Ajax::updateItemOnSelectEvent('solutiontypes_id'.$rand_type, "show_solutiontemplates_by_solutiontypes",
													$CFG_GLPI["root_doc"]."/ajax/dropdownSolutionTemplate.php",
													$opt);
         echo "</span>";
			//--
      } else {
      ...
      if ($show_template) {
         ...
         echo "<td>".$LANG['jobresolution'][6]."&nbsp;:&nbsp;</td><td>";
			...
			/*//SUPRISERVICE*/
         echo "<span id='show_solutiontemplates_by_solutiontypes'>";
			/*
         Dropdown::show('SolutionTemplate',
			...
			*/
			...
			$solutiontype->getFromDB($this->getField('solutiontypes_id'));
			SolutionTemplate::dropdownBySolutionType($solutiontype, 'solutiontemplates_id'.$rand_template, $this->getField('solutiontemplates_id'));
			$opt = array('value' => '__VALUE__', 'type_id' => 'solutiontypes_id'.$rand_type);
			Ajax::updateItemOnSelectEvent("solutiontemplates_id".$rand_template, 'solution'.$rand_text,
													$CFG_GLPI["root_doc"]."/ajax/solution.php",
													$opt);
         echo "</span>";
   ...
   function showSolutionFormMobile($knowbase_id_toload=0) {
      global $LANG, $CFG_GLPI;
      ...
      echo "<tr><th colspan='".$params['colspan']."' width='200px'>";
      ...
      echo "</th></tr>\n";
		//ShowFormHeader
		//echo "<input type='submit' name='update' value='teste' class='submit' onclick='alert(\"teste\" + document.forms[0]);'>";
...
//                        && empty($this->fields['solution']);
      ...
      if ($canedit && Session::haveRight('knowbase','w')) {
         ...
         //var_export($this->fields['type_fechamento']);
        // die();
   ...
   **/
   static function getAllTypesForHelpdesk() {
      ...
      if (isset($PLUGIN_HOOKS['assign_to_ticket'])) {
       ...
       $sess = $_SESSION["glpiactiveprofile"]["helpdesk_item_type"];                
       $trans = array("Transport");
       $glpiactive = array_merge($sess, $trans);

      //Types of the core (after the plugin for robustness)
      foreach($CFG_GLPI["ticket_types"] as $itemtype) {
         if ($item = getItemForItemtype($itemtype)) {
            if (!isPluginItemType($itemtype) // No plugin here
                && in_array($itemtype,$glpiactive)) {
   ...
   **/
   static function isPossibleToAssignType($itemtype) {
      ...
      // standard case
            ...
            $trans = array("Transport",'ConsumableItem');
            $glpiactive = array_merge_recursive($sess, $trans);
          
          
         if (in_array($itemtype, $glpiactive)) {
            return true;
         }
      }
