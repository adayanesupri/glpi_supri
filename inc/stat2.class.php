        ...
        switch ($type) {
            ...
            case "inter_avgtakeaccount" :
                ...
                $query = "SELECT `$table`.`id`,
                      ...
                      ORDER BY `$table`.`solvedate`";
                /////////////////////////////////////////////////// 
                /* if($_SESSION["glpiname"] == 'alucas'){ */
                $result = $DB->query($query);
                //if ($result && $DB->numrows($result)>0) {
                if ($DB->numrows($result) > 0)
                    mysql_data_seek($result, 0);
                while ($row = $DB->fetch_array($result)) {
                    $datas[$row['id']] = array("inicial" => $row['date'], "final" => $row['solvedate']);
                }
                ...
                if ($DB->numrows($result) > 0)
                    mysql_data_seek($result, 0);
                while ($row = $DB->fetch_array($result)) {
                    // var_export(array($datas[$row['id']]['inicial'],$datas[$row['id']]['final']));
                    $datas[$row['id']]['dt_inicio'] = $datas[$row['id']]['inicial'];
                    $datas[$row['id']]['dt_final'] = $datas[$row['id']]['final'];
                    while ($datas[$row['id']]['inicial'] <= $datas[$row['id']]['final']) {

                        $sql = "SELECT DATE_ADD('{$datas[$row['id']]['inicial']}', INTERVAL {$i} DAY) AS data;";
                        $result_sql = $DB->query($sql);
                        $row_sql = $DB->fetch_array($result_sql);

                        if ($row_sql['data'] <= $datas[$row['id']]['final']) {

                            //pegar dia da semana, para nao somar finais de semana
                            $diaa = substr($datas[$row['id']]['inicial'], 8, 2) . "-";
                            $mes = substr($datas[$row['id']]['inicial'], 5, 2) . "-";
                            $ano = substr($datas[$row['id']]['inicial'], 0, 4);
                            $dia_semana = date("w", mktime(0, 0, 0, $mes, $diaa, $ano));

                            if ($dia_semana != 5 && $dia_semana != 6) {
                                $intervalo[$row['id']][] = $row_sql['data'] . "-" . $dia_semana;                                
                            }
                        }

                        $datas[$row['id']]['inicial'] = $row_sql['data'];
                    }
                }
                ...
                while ($row = $DB->fetch_array($result)) {
                    if ($intervalo[$row['id']]) {
                        $contador = count($intervalo[$row['id']]);
                        $horas_intervalo = $contador * 9;
                        $horas_intervalo = "{$horas_intervalo}:00:00";
                        $intervalo[$row['id']]['total'] = $horas_intervalo;
                        $total[$row['id']]['intervalo'] = $intervalo[$row['id']]['total'];
                        //var_export($horas_intervalo);
                    }
                    //var_export($datas[$row['id']]['dt_inicio']);
                    //die();
                    $hora_inicio = substr($datas[$row['id']]['dt_inicio'], 11, 2) . ":";
                    $min_inicio = substr($datas[$row['id']]['dt_inicio'], 14, 2);

                    $diaa = substr($datas[$row['id']]['dt_inicio'], 8, 2);
                    $mes = substr($datas[$row['id']]['dt_inicio'], 5, 2) . "-";
                    $ano = substr($datas[$row['id']]['dt_inicio'], 0, 4);

                    $total_inicio = "$hora_inicio$min_inicio:00";
 
                    $unix_data1 = strtotime($datas[$row['id']]['dt_inicio']);
                    $unix_data2 = strtotime("$ano-$mes$diaa 18:00:00");

                    $nHoras = ($unix_data2 - $unix_data1) / 3600;
                    $nMinutos = (($unix_data2 - $unix_data1) % 3600) / 60;

                    $total[$row['id']]['hsinicio'] = sprintf('%02d:%02d', $nHoras, $nMinutos);

                    if ($total_inicio < '12:00:00') {
                        $var = substr($total[$row['id']]['hsinicio'], 0, 2);
                        $var = $var - 1;
                        $var2 = substr($total[$row['id']]['hsinicio'], 3, 2);
                        $total[$row['id']]['hsinicio'] = "$var:$var2";

                    }
                    
                    $hora_final = substr($datas[$row['id']]['dt_final'], 11, 2) . ":";
                    $min_final = substr($datas[$row['id']]['dt_final'], 14, 2);

                    $diaa_f = substr($datas[$row['id']]['dt_final'], 8, 2);
                    $mes_f = substr($datas[$row['id']]['dt_final'], 5, 2) . "-";
                    $ano_f = substr($datas[$row['id']]['dt_final'], 0, 4);

                    $total_final = "$hora_final$min_final:00";
                    
                    $unix_data3 = strtotime("$ano_f-$mes_f$diaa_f 8:00:00");
                    $unix_data4 = strtotime($datas[$row['id']]['dt_final']);

                    $nHoras_f = ($unix_data4 - $unix_data3) / 3600;
                    $nMinutos_f = (($unix_data4 - $unix_data3) % 3600) / 60;

                    $total[$row['id']]['hsfinal'] = sprintf('%02d:%02d', $nHoras_f, $nMinutos_f);
                    
                    if ($total_final > '13:00:00') {
                        
                        $var = substr($total[$row['id']]['hsfinal'], 0, 2);
                        $var = $var - 1;
                        $var2 = substr($total[$row['id']]['hsfinal'], 3, 2);
                        $total[$row['id']]['hsfinal'] = "$var:$var2";                       
                ...
                while ($row = $DB->fetch_array($result)) {
                    $atd = new Atendimento();
                    $array_soma[] = $atd->somaHoras($total[$row['id']]);
                    
                }
                return $atd->somaHoras($array_soma);
                
                //var_export($atd->somaHoras($array_soma));
                //die();
