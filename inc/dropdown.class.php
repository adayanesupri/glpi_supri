<?php
/*
 * @version $Id: dropdown.class.php 18684 2012-06-22 08:06:15Z tsmr $
 -------------------------------------------------------------------------
   ...
   **/
   static function getMassiveActions($itemtype, $is_deleted=0) {
      ...
      $actions = array();
		...
		/*//SUPRISERVICE*/
      if ($itemtype == 'Fusioninventory_Printerlogs') {
         $actions['delete']    = $LANG['buttons'][6];
		} else if ($itemtype == 'Contract_Item_Printer_Supridesk') {
         $actions['massive_move_to_group']   = $LANG['agrupamento'][15];
      } else if ($itemtype == 'Contract_Item_Supridesk') {
			$actions['update'] = $LANG['buttons'][14];
      } else if ($itemtype == 'ITILCategory_SolutionTypes') {
         $actions['desvincular_diagnostico']   = $LANG['custom_chamado'][5];
      } else if ($itemtype == 'SolutionType_SolutionTemplates') {
         $actions['desvincular_solucao']   = $LANG['custom_chamado'][5];
      } else if ($itemtype == 'Tarifacao_Supridesk') {
         $actions['delete']    = $LANG['buttons'][6];
         $actions['faturar']   = $LANG['tarifacao'][53];
               ...
               case 'Ticket' :
                  ...
                  if (Session::haveRight("update_ticket","1")) {
                     $actions['add_actor']   = $LANG['job'][27];
							/*//SUPRISERVICE*/
                     $actions['overwrite_actor'] = $LANG['supriservice'][6];
