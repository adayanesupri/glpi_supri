<?php
/*
 * @version $Id: entity.class.php 18307 2012-04-18 12:37:14Z moyo $
 -------------------------------------------------------------------------
...
class Entity extends CommonTreeDropdown {
   ...
   function defineTabs($options=array()) {
      ...
      $this->addStandardTab($this->getType(), $ong, $options);
		/*//SUPRISERVICE*/
      $this->addStandardTab('EntityMap', $ong, $options);
   ...
   **/
   function executeAddRule($input) {
	...
	/*//SUPRISERVICE*/
   static function getEntitiesMaps( $array_entities, $delayed ) {
      global $DB, $CFG_GLPI;

		$entities_id = implode(",", $array_entities);
	
      $entities = array();

		if ( isset($delayed) && $delayed == 1 && Session::haveRight("contract", "w"))
		{
			$query = "	SELECT e.id, e.completename, ed.latitude, ed.longitude, ed.town
							FROM glpi_tickets t, glpi_entities e
							LEFT JOIN glpi_entitydatas ed
								  ON (ed.entities_id = e.id)
							WHERE t.due_date < now()
							AND t.status NOT IN ('solved', 'closed', 'canceled')
							AND e.id IN ( {$entities_id} )
							AND t.entities_id = e.id";
		}
		elseif ( isset($delayed) && $delayed == 0)
		{
			$query = "	SELECT e.id, e.completename, ed.latitude, ed.longitude, ed.town
							FROM glpi_tickets t, glpi_entities e
							LEFT JOIN glpi_entitydatas ed
								  ON (ed.entities_id = e.id)
							WHERE t.status NOT IN ('solved', 'closed', 'canceled')
							AND e.id IN ( {$entities_id} )
							AND t.entities_id = e.id";
		}
		else
		{
			...
			$query = "SELECT glpi_entities.id, glpi_entities.completename, latitude, longitude, town
						 FROM `glpi_entities`
						 LEFT JOIN `glpi_entitydatas`
								ON (`glpi_entitydatas`.`entities_id` = `glpi_entities`.`id`)
						WHERE glpi_entitydatas.latitude is not null
						AND  glpi_entitydatas.longitude is not null
						AND glpi_entities.id IN ( {$entities_id} )
