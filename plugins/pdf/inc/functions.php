...
function plugin_pdf_add_header(PluginPdfSimplePDF $pdf, $ID, CommonDBTM $item) {
   ...
   if ($item->getFromDB($ID) && $item->can($ID,'r')) {
      ...
      $pdf->setHeader(" <b>CHAMADO $name</b> ");
...
function plugin_pdf_main_ticket(PluginPdfSimplePDF $pdf, Ticket $job) {
   ...
   $entitydata = new EntityData();
	if ($entitydata->getFromDB($job->fields['entities_id']))
	{
	   $pdf->setColumnsSize(100);
   	   $address = $entitydata->getfield('name') . ' - ' . $entitydata->getfield('town') . ' ' . $entitydata->getfield('state') . ' - CEP: ' . $entitydata->getfield('postcode');
	}

	$entity = new Entity();
		if ($entity->getFromDB($job->fields['entities_id'])){
			$entidade = $entity->getfield('completename');
			$pdf->displayTitle("<b>$entidade</b> ");
		}
   ...
   $pdf->displayLine("<b><i>".$LANG['joblist'][11]."</i></b> : ".convDateTime($job->fields["date"]).", ".
                     ...
                     "<b><i>".$LANG['common'][26]."</i></b> : ".convDateTime($job->fields["date_mod"]));
   ...
   if ($job->fields["itemtype"] && class_exists($job->fields["itemtype"])) {
      ...
      if ($item->getFromDB($job->fields["items_id"])) {
		 ...
		 if (isset($item->fields["serial"])) {
            $model_item = html_clean(Dropdown::getDropdownName('glpi_'.html_clean($job->fields["itemtype"]).'models', html_clean($item->fields["printermodels_id"]))) ;
         }
      ...
      $pdf->displayText(
         ...
         html_clean($item->getTypeName())." $model_item ".$serial_item . $otherserial_item ,1);
   } else {
   ...
   $entitydata = new EntityData();
	...
	{
	   $pdf->setColumnsSize(100);
   	   $address = $entitydata->getfield('address') . ' - ' . $entitydata->getfield('town') . ' ' . $entitydata->getfield('state') . ' - CEP: ' . $entitydata->getfield('postcode');
	   $pdf->displayLine("<b><i>Endereço:</i></b> : " . $address);
	}
...
function plugin_pdf_solution(PluginPdfSimplePDF $pdf, Ticket $job) {
   ...
   if ($job->fields['ticketsolutiontypes_id'] || !empty($job->fields['solution'])) {
	  ...
	  $pdf->displayLine("");
	  $pdf->displayLine("");
	  if (html_clean(Dropdown::getDropdownName("glpi_ticketcategories",$job->fields["ticketcategories_id"])) == "Troca de Toner") {
	  $item = new $job->fields["itemtype"]();
	  ...
	  $query = "SELECT distinct glpi_cartridgeitems.name 
				FROM `glpi_cartridgeitems` ,glpi_cartridges,glpi_printers,glpi_printermodels
				WHERE glpi_cartridgeitems.id = glpi_cartridges.cartridgeitems_id
				and glpi_cartridges.printers_id = glpi_printers.id
				and glpi_printers.printermodels_id = glpi_printermodels.id
				and glpi_printermodels.name = '$model_item'";
	...
	$pdf->displayTitle("<b>Suprimentos trocados</b>");
	  ...
	  $pdf->setColumnsSize(33,33.3,34);
	  
	 if ($arr[1]){
	     $pdf->displayLine("$arr[0]","$arr[1]","$arr[2]");	
	 }
	 if ($arr[3]){
     $pdf->displayLine("$arr[3]","$arr[4]","$arr[5]");	
	 }
...
function plugin_pdf_satisfaction(PluginPdfSimplePDF $pdf, Ticket $ticket) {
   global $LANG;
   ...
   if (!$survey->getFromDB($ticket->getID())) {                   // No survey
      ...
      $pdf->displayLine('<b>'.$LANG['satisfaction'][6].'</b> : '.
                        convDateTime($survey->getField('date_begin')));
      $pdf->displayLine('<b>'.$LANG['satisfaction'][4].'</b> : '.
                        convDateTime($survey->getField('date_answered')));
      $pdf->displayLine('<b>'.$LANG['satisfaction'][1].'</b> : '.$sat);
      $pdf->displayText('<b>'.$LANG['common'][25].'</b> : ',
                        $survey->getField('comment'));

   } else {                                                       // No answer
      $pdf->displayLine($LANG['satisfaction'][6].' : '.convDateTime($survey->getField('date_begin')));
...
function plugin_pdf_cartridges(PluginPdfSimplePDF $pdf, Printer $p, $old=false) {
   ...
   if (!$number) {
      ...
      while ($data=$DB->fetch_array($result)) {
         ...
         $col2 = Cartridge::getStatus($data["date_use"], $data["date_out"], null, null);
         $col6 = '';
         ...
         case 'Ticket' :
            ...
            foreach ($tab as $i) {
               switch ($i) { // Value not from Job::defineTabs but from plugin_pdf_prefPDF
                  ...
                  case 10 :
                     plugin_pdf_satisfaction($pdf, $item);
                     break;
