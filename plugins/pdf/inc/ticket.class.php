...
/*
 * @version $Id: ticket.class.php 314 2012-01-31 15:48:22Z remi $
 -------------------------------------------------------------------------
...
class PluginPdfTicket extends PluginPdfCommon {
   ...
   static function pdfMain(PluginPdfSimplePDF $pdf, Ticket $job) {
      ...
      if (count($_SESSION['glpiactiveentities'])>1) {
		...
		/*//SUPRISERVICE*/
		$pdf->displayTitleCustom($entity);
	  
		$entitydata = new EntityData();
      $entitydata->getFromDB($job->fields["entities_id"]);
	  
	     
		$pdf->displayLine("<b><i>Endereco:</b></i> ".$entitydata->getfield('address').' '. $entitydata->getfield('town'). ' ' . $entitydata->getfield('state') . ' - CEP: ' . $entitydata->getfield('postcode') ); 
      ...
      if ($job->fields["itemtype"] && ($item = getItemForItemtype($job->fields["itemtype"]))) {
         if ($item->getFromDB($job->fields["items_id"])) {
			...
			//Alteração Marcelo -- Exibir modelo
			$tabela_modelo = strtolower("glpi_".$job->fields["itemtype"]."models");
			$coluna_id_modelo = strtolower($job->fields["itemtype"]."models_id");
         $modelo = ", <b><i>Modelo</i></b> : ".Dropdown::getDropdownName($tabela_modelo, Html::clean($item->fields[$coluna_id_modelo]));
         ...
         $pdf->displayText(
            ...
            Html::clean($item->getTypeName())."  ".Html::clean($item->getNameID())."$modelo".
                  $serial_item . $otherserial_item ,
            1);
      } else {
      ...
      $pdf->displayText("<b><i>".$LANG['joblist'][6]."</i></b> : ", $job->fields['content']);
		/*//SUPRISERVICE*/
      //$pdf->displayText("<b>ATENÇÃO CLIENTE:  </b>", "A Supriservice Informática LTDA não se responsabiliza pelos dados contidos em discos rígidos caso seja necessário trocá-lo ou fazer qualquer outra manutenção. Certifique-se que realizou seus procedimentos de backups, se necessário, antes de continuar o atendimento deste RAT.", 1, 3);
//      $pdf->displaySpace();
   }
   ...
   static function pdfSolution(PluginPdfSimplePDF $pdf, Ticket $job) {
      ...
      if ($job->fields['solutiontypes_id'] || !empty($job->fields['solution'])) {
         ...
         $pdf->displayText("<b><i>$title</i></b> : ", $sol);
			...
			/*//SUPRISERVICE*/
			//Cálculo da quantidade de linhas em branco do campo abaixo
			$countLinhasQuebradas = substr_count($job->fields['content'], "\r\n");
			$linhas = explode( "\r\n", $job->fields['content'] );
			$countLinhasDescricao = 0;
			foreach ( $linhas as $linha )
			{
				$linhasTexto = ceil(strlen($linha) / 135) > 0 ? ceil(strlen($linha) / 135) : 1;
				$countLinhasDescricao += $linhasTexto;
			}
			$countLinhas = max( $countLinhasQuebradas , $countLinhasDescricao );
			$countLinhas += $countLinhasDescricao == $countLinhasQuebradas ? 1 : 0;
			$fatorLinhaDescricaoXLinhaBranco = 0.45; //cada 3 linhas na descrição equivalem a 2 linhas em branco, seria 0.66 mas foi dada uma margem de erro
			
                        // Teste: Se tiver componente associado, calcula o número de linhas
                        $produto = $job->getActiveCartridgeItem($job->fields['id'], 1);
                        if ( $produto != null ) {
                            $aux = 1;
                        }
                        $totalLinhasBranco = (20 - $aux) - ceil($countLinhas * $fatorLinhaDescricaoXLinhaBranco);

			//Alteração Marcelo - quando não houver solução imprimir campos em branco para preenchimento
			
                        //$pdf->displayLine("");
                        $pdf->displayLine("<b><i>     Data:  ______/______/____________            Hora de Inicio: ________:________              Hora de Termino: ________:________</b></i>");
                        
				for ( $i = 0; $i < $totalLinhasBranco; $i++ )
					$pdf->displayLine("");
                        
                        $pdf->displayLine("<b><i>     Técnico (nome legível):</b></i>");
                        //$pdf->displayLine("<b><i>     Técnico (nome legível):_______________________________________________________________________________________</b></i>");
			$pdf->displayTitle("<b>Aceite dos servicos</b>");
			$pdf->displayLine("<b><i>Nome:</b></i>");
			$pdf->displayLine("<b><i>Carimbo e Assinatura:</b></i>");
		}
	}


   static function pdfEstoque(PluginPdfSimplePDF $pdf, Ticket $job) {
      global $LANG, $CFG_GLPI, $DB;
		...
		if ( $produto != null )
		{
			$pdf->displayTitle("<b>Requisição de Suprimento</b>");
			$pdf->displayLine("<b><i>Produto:</i></b> {$produto}" );
			$pdf->displaySpace();
		}
	}
	...
	//--
	 ...
	 function defineAllTabs($options=array()) {
      global $LANG;
		...
		/*//SUPRISERVICE - não foi mais necessário criação da aba, comentado para caso necessite de uso futuro*/
      $onglets = parent::defineAllTabs($options);
		//$onglets  = array_merge( array('_estoque_' => "Requisição de Suprimento"), parent::defineAllTabs($options));
		//$onglets = $this->array_swap_assoc('_estoque_', '_main_', $onglets);
		//--
