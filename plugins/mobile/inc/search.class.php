...
class PluginMobileSearch extends Search {
   ...
   static function show ($itemtype) {
      ...
      return PluginMobileSearch::showList($itemtype,$_GET);
   }
         ...
         foreach ($options as $key => $val) {
            ...
            if (!is_array($val)) {
               ...
               if (!isset($val['nosearch']) || $val['nosearch']==false) {
                  ...
                  echo ">". substr($val["name"],0,28) ."</option>";
               }
            }
         }
         if (!$first_group) {
      ...
      if (is_array($linked) && count($linked)>0) {
         for ($i=0 ; $i<$_SESSION["glpisearchcount2"][$itemtype] ; $i++) {
            ...
            foreach ($linked as $key) {
               ...
               echo "<option value='$key'>".substr($metanames[$key],0,20)."</option>\n";
            }
            echo "</select>";
   ...
   static function showList ($itemtype,$params) {
      ...
      if ($itemtype!='States' && class_exists($itemtype)) {
		...
		/*//SUPRISERVICE*/
      $_SESSION['plugin_mobile']['rows_limit']=20; // sdb38l
   ...
   *
   **/
   static function giveItem ($itemtype,$ID,$data,$num,$meta=0, array $addobjetparams=array()) {
      ...
      switch ($table.'.'.$field) {
         ...
         case "glpi_users.name" :
            ...
            if ($itemtype != 'User' && isset($searchopt[$ID]["forcegroupby"]) && $searchopt[$ID]["forcegroupby"]) {
               ...
               if (!empty($linkfield)) {
                  ...
                  return formatUserNameMobile($data[$NAME.$num."_3"],$data[$NAME.$num],$data[$NAME.$num."_2"],
         ...
         case "glpi_tickets.count" :
            if ($data[$NAME.$num]>0 && haveRight("show_all_ticket","1")) {
               ...
               $out= "<a href=\"".$CFG_GLPI["root_doc"]."/front/ticket.php?".append_params($options)."\" data-back='false'>";
               $out .= $data[$NAME.$num];
               $out .= "</a>";
            } else {
               $out= $data[$NAME.$num];
