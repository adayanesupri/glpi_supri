...
function navigatorDetect() {
   if (isset($_SERVER['HTTP_USER_AGENT'])) {
      ...
      elseif (stripos($_SERVER['HTTP_USER_AGENT'], 'Chrome')) return "Desktop";
		/*//SUPRISERVICE*/
      elseif (stripos($_SERVER['HTTP_USER_AGENT'], 'IEMobile')) return "Android";
      else return "default";
   }
   return;
}
...
function saveActiveProfileAndApplyRead() {
   ...
   foreach($_SESSION['temp_glpiactiveprofile'] as &$val) {
		/*//SUPRISERVICE */ //gravar temporariamente quais $val foram modificadas para poder restaurar na funcao restoreActiveProfile
      //if ($val == 'w') $val = 'r';
   }
}
...
function nativeSelect() {
   if (isset($_SESSION['plugin_mobile']['native_select'])
      && $_SESSION['plugin_mobile']['native_select']== 1) return true;
   return false;
}
