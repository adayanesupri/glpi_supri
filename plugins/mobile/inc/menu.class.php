...
class PluginMobileMenu extends CommonDBTM {
   ...
   public function showFirstLevel($message) {
      ...
      if ($message != '') {
		...
		/*//SUPRISERVICE*/
		$rand = mt_rand();
		$options['name']     = "to_entity";
		$options['comments'] = "";
		$options['rand']     = $rand;
		$options['toupdate'] = array('value_fieldname' => 'value',
											  'to_update'  => "notif_user_$rand",
											  'url'        => $CFG_GLPI["root_doc"]."/plugins/mobile/ajax/activeentity.php");

		//echo "<center>";
		echo "<br><ul data-role='listview' data-inset='true' data-theme='c' data-dividertheme='a'>";
      echo "<li data-role='list-divider'>Entidade ativa</li>";
		//echo "</center>";
      ...
      echo "</ul>";
		...
		echo "<span id='notif_user_$rand' style='padding:10px;width:90%;'></span>";
		/**/
   ...
   public function getMenu($sector="none",$item="none",$option="") {
      ...
      // ASSISTANCE
		/*//SUPRISERVICE*/
		$countPendentes = Ticket::countTicketsForUser(Session::getLoginUserID());
		$pendentes = $countPendentes > 0 ? ' ('.Ticket::countTicketsForUser(Session::getLoginUserID()).' em aberto)' : '';
      $menu['maintain']['title']=$LANG['title'][24] . $pendentes;
      $menu['maintain']['default']='/front/ticket.php';
      $menu['maintain']['icon']='icons/assistance.png';

      if (Session::haveRight("observe_ticket","1") || Session::haveRight("show_all_ticket","1")
          || Session::haveRight("create_ticket","1")) {

         $menu['maintain']['content']['own_tickets']['title']='Seus Chamados';
         $menu['maintain']['content']['own_tickets']['icon']='icons/ticket.png';
         $menu['maintain']['content']['own_tickets']['shortcut']='t';
         $menu['maintain']['content']['own_tickets']['page']='/front/search.php?itemtype=ticket&menu=maintain&ssmenu=ticket';
         $menu['maintain']['content']['own_tickets']['links']['search']='/front/ticket.php';

         if (Session::haveRight('validate_ticket',1)) {
            $opt=array();
            $opt['reset']  = 'reset';
            $opt['field'][0]      = 12; // validation status
            $opt['searchtype'][0] = 'equals';
            $opt['contains'][0]   = 'notold';
            $opt['link'][0]        = 'AND';

            $opt['field'][1]      = 5; // validation aprobator
            ...
            $menu['maintain']['content']['own_tickets']['links'][$pic_validate]='/front/ticket.php?'.Toolbox::append_params($opt,'&amp;');
				$menu['maintain']['content']['own_tickets']['page'].= '&'.Toolbox::append_params($opt,'&amp;');

				//print $menu['maintain']['content']['own_tickets']['page'] . "<br><br>";
				//print http_build_query($opt);
				$menu['maintain']['content']['own_tickets']['page'] = '/front/search.php?itemtype=ticket&menu=maintain&ssmenu=ticket&'.http_build_query($opt);
				//$menu['maintain']['content']['own_tickets']['page']='/front/ticket.php?'.Toolbox::append_params($opt,'&amp;');
         }
      }
		//--
      ...
      if (Session::haveRight("show_planning","1") || Session::haveRight("show_all_planning","1")) {
         $menu['maintain']['content']['planning']['title']=$LANG['Menu'][29]; //* Tratar essa situação específica
      ...
      /*if (Session::haveRight("rule_ldap","r")
         ...
         if ($sector=='admin' && $item == 'rule') {
            ...
            if (Session::haveRight("rule_ocs","w")) {
            ...
            if (Session::haveRight("rule_ldap","w")) {
            ...
            if (Session::haveRight("rule_mailcollector","w")) {
            ...
            if (Session::haveRight("entity_rule_ticket","w")) {
            ...
            if (Session::haveRight("rule_softwarecategories","w")) {
               $menu['admin']['content']['rule']['options']['softwarecategories']['links']['add']='/front/rulesoftwarecategory.form.php';
            }
         }
      }

      if (Session::haveRight("rule_dictionnary_dropdown","r") || Session::haveRight("rule_dictionnary_software","r")) {
      ...
      if (Session::haveRight("config","w") || Session::haveRight("notification","r")) {
