...
class PluginMobileItem extends CommonDBTM {
   ...
   public function displayItem($ID, $itemtype, $params = array()) {
      ...
      echo "<form action='".$CFG_GLPI["root_doc"]."/plugins/mobile/front/item.form.php?menu="
         .$_GET['menu']."&ssmenu=".$_GET['ssmenu']."&itemtype=$itemtype&id=$ID"
         ."' method='post'>";
      ...
      if (largeScreen()) {
         ...
         /*echo count($labels1)."<br>";
         echo count($labels2)."<br>";*/
         
         echo "<div class='ui-grid-a' id='tablet-grid'>";
         echo "<div class='ui-block-a'>";
         $this->showList($labels1, $itemtype, $ID);
         echo "</div>";
         echo "<div class='ui-block-b'>";
         $this->showList($labels2, $itemtype, $ID);
         echo "</div>";
         echo "</div>";
      } else {
         ...
         unset($labels['id']);
         $this->showList($labels, $itemtype, $ID);
      }
   ...
   public function showList($fields, $itemtype, $ID) {
      $classname = ucfirst($itemtype);
      $obj = new $classname;
      $table = $obj->getTable();
      
		/*//SUPRISERVICE*/
		$isTicket = $itemtype == 'Ticket' ? true : false;
      ...
      foreach($fields as $key => $row) {
         ...
         $itemSearchOptions = $searchOptions[$this->getOptionNumber($searchOptions, $itemtype, $key, $fields[$key."_label"])];
			/*//SUPRISERVICE*/
			if ( array_key_exists( 'mobile_readonly', $itemSearchOptions) )
				$readonlyFields[] = $key;

			$css_class = "";
			if ( array_key_exists( 'css_class', $itemSearchOptions) )
				$css_class = "class='" . $itemSearchOptions['css_class'] . "' id='{$key}opt'";
            ...
            if ($itemSearchOptions['table'] != $table) {
               ...
               echo "<input $css_class type='text' data-role='date' name='$key' value='$row' />";
				/*//SUPRISERVICE*/
            } elseif(isset($itemSearchOptions['datatype']) && ( $itemSearchOptions['datatype'] == "datetime2" || $itemSearchOptions['datatype'] == "datetime" ) ) {
               echo "<input {$css_class} type='text' readonly data-role='date' name='$key' value='$row' />";
            } else {
					/*//SUPRISERVICE*/
					if ( $itemSearchOptions['field'] == 'items_id')
					{
						$tt = new TicketTemplate();
						$itemtype = $fields['itemtype'];
						$items_id = $fields['items_id'];

						if ($itemtype && ($item = getItemForItemtype($itemtype)) && $items_id) {
							if ($item->can($items_id,'r')) {
								echo $item->getTypeName()." - ".$item->fields['otherserial']." - ".$item->fields['serial']; //antigo: $item->getLink(true);
							} else {
								echo $item->getTypeName()." - ".$item->fields['otherserial']." - ".$item->fields['serial']; //$item->getNameID();
							}
						}
					}else{
						echo "<input type='text' name='$key' value='$row' />";
					}
				}
         } else {
            if ($itemSearchOptions['table'] != $table) {
...
MOBISCROLL;
      ...
      echo "</ul>";
		...
		if ( $isTicket )
		{
			$ticket = new Ticket();
			$ticket->getFromDB($ID);
			$options = Array( "show_tooltip" => false );
			$ticket->showActorsPartForm($ID, $options);
		}
   }
   ...
   public function getLabels($params, &$output) {
      ...
      foreach($p['searchOptions'] as $id => $option) {
         if (isset($option['table'])) {
            ...
            if (array_key_exists($linkfield, $p['data']) && !array_key_exists('mobile_hide', $option)
                  //&& $p['data'][$linkfield] != ''
   ...
   public function getOptionNumber($opts, $itemtype, $field_key, $label) {
      foreach ($opts as $num => $opt) {
			/*//SUPRISERVICE*/
         if ((isset($opt['linkfield']) && $opt['linkfield']==$field_key && $opt['name']==$label) || (isset($opt['field']) && $opt['field']==$field_key)) {
            return $num;
         }
      }
