...
/*
 * @version $Id: methodsession.class.php 306 2011-11-08 12:36:05Z remi $
 -------------------------------------------------------------------------
   ...
   **/
   static function methodLogin($params, $protocol) {
      ...
      if ($identificat->Login($params['login_name'], $params['login_password'], true)) {
			...
			if ( $user->fields['mobidesk_code'] != $params['deviceID'] )
				return self::Error($protocol, WEBSERVICES_ERROR_LOGINFAILED, '', Html::clean('Dispositivo não autorizado'));
			//--
      ...
      if ($plug->isActivated('webservices')) {
         ...
         if (!Session::getLoginUserID() && !count($entities)) {
            ...
            Toolbox::logInFile(LOGFILENAME,
                               $LANG['login'][5] . " ($username, $iptxt, $method, $protocol)\n");
         } else { // Allowed
            if (!Session::getLoginUserID()) {
            ...
            if (is_numeric($log)) {
               ...
               Toolbox::logInFile($log, $LANG['log'][55] . " ($username, $iptxt, $method)\n");
            }
         ...
         if ($debug) {
            Toolbox::logInFile(LOGFILENAME, $LANG['log'][55] . ": $username, $iptxt\n".
