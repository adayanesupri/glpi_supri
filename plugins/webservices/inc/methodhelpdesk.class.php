...
/*
 * @version $Id: methodhelpdesk.class.php 334 2012-06-26 08:29:11Z yllen $
 -------------------------------------------------------------------------
...
class PluginWebservicesMethodHelpdesk extends PluginWebservicesMethodCommon {
   ...
   **/
   static function methodListHelpdeskItems($params, $protocol) {
      ...
      if ($type && $type !='my' && ($item = getItemForItemtype($type))
          && ($_SESSION["glpiactiveprofile"]["helpdesk_hardware"]
          & pow(2, HELPDESK_ALL_HARDWARE))
          && Ticket::isPossibleToAssignType($type)) {
         // All items of a type
         $table = getTableForItemType($type);
   ...
   **/
   static function methodGetTicket($params, $protocol) {
      ...
      // Users & Groups
      $tabtmp = array(Ticket::REQUESTER => 'requester',
   ...
   **/
   static function methodAddTicketDocument($params, $protocol) {
	...
	/*//SUPRISERVICE*/
   	static function methodGetStaticData($params, $protocol) {
		global $DB, $LANG, $CFG_GLPI;

		$data = array();
	  
		$query = "SELECT * FROM glpi_itilcategories WHERE is_deleted = 0 ORDER BY name";
		foreach ($DB->request($query) as $row) { $data['glpi_itilcategories'][] = $row; }

		$query = "SELECT * FROM supridesk_itilcategories_solutiontypes";
		foreach ($DB->request($query) as $row) { $data['supridesk_itilcategories_solutiontypes'][] = $row; }

		$query = "SELECT * FROM glpi_solutiontypes";
		foreach ($DB->request($query) as $row) { $data['glpi_solutiontypes'][] = $row; }

		$query = "SELECT * FROM supridesk_solutiontypes_solutiontemplates";
		foreach ($DB->request($query) as $row) { $data['supridesk_solutiontypes_solutiontemplates'][] = $row; }

		$query = "SELECT * FROM glpi_solutiontemplates";
		foreach ($DB->request($query) as $row) { $data['glpi_solutiontemplates'][] = $row; }
		
		return $data;
   }
   
   static function methodSyncTickets($params, $protocol) {
      global $DB, $LANG, $CFG_GLPI;
      ...
      if (!isset($params['tickets'])) {
         return self::Error($protocol, WEBSERVICES_ERROR_MISSINGPARAMETER, '', 'tickets');
      }
      ...
      $ticket = new Ticket();
		...
		//--
		for ( $i = 0; $i <= count($params['tickets']) ; $i++)
		{
			for( $j = 0; $j < count($params['tickets'][$i]['log']); $j++)
			{
				$date_mod = str_replace("'", "", $params['tickets'][$i]['log'][$j]['date_mod']);
				if ( strtotime($user->fields['mobidesk_code_mod']) > strtotime($date_mod) )
				{
		         return self::Error($protocol, WEBSERVICES_ERROR_FAILED, '', 'Dados do dispositivo são muito antigos para serem sincronizados.');
				}
			}
		}
		...
		for ( $i = 0; $i <= count($params['tickets']) ; $i++)
		{
			//se não puder alterar este ticket, pula.
			if ( !$ticket->can( $params['tickets'][$i]['data']['id'],'w') )
				continue;

			//verifica os campos alteráveis
			$items_id = $params['tickets'][$i]['data']['id'];

			//se for campo status
			$status_date_mod = $params['tickets'][$i]['data']['status_mod'];
			$query = "	SELECT *
							FROM `glpi_logs`
							WHERE `items_id` = '$items_id'
								AND `itemtype` = 'Ticket'
								AND id_search_option = 12
								AND date_mod > '$status_date_mod' ";
			$result=$DB->query($query);
			if ($DB->numrows($result) == 0)
			{
				//status foi atualizado mais recentemente no Mobile, então atualiza dados do Supridesk
				$input = array('status'						=> $params['tickets'][$i]['data']['status'],
								'id'						=> $items_id,
								'_disablenotif'				=> true,
								'_doNotUpdateIfSolved'		=> true
							);
							
				if ( $params['tickets'][$i]['data']['status'] == 'solved' )
				{
					$input['solutiontypes_id'] = $params['tickets'][$i]['data']['solutiontypes_id'];
					$input['solution_itilcategories_id'] = $params['tickets'][$i]['data']['solution_itilcategories_id'];
					$input['solutiontemplates_id'] = $params['tickets'][$i]['data']['solutiontemplates_id'];
					$input['solution'] = $params['tickets'][$i]['data']['solution'];
					$input['solvedate'] = $params['tickets'][$i]['data']['solvedate'];
				}
				$ticket->update($input, 0);
			}

			$requesttypes_id = 0; //default 0 pelo banco
			foreach ($DB->request('glpi_requesttypes', array('name' => 'Supridesk Mobile')) as $data) {
				$requesttypes_id = $data['id'];
			}
			
			//importa followups novos do dispositivo
			for( $j = 0; $j < count($params['tickets'][$i]['followups']); $j++)
			{
				//se não for id nulo (novo registro no dispositivo), pula...
				if ( $params['tickets'][$i]['followups'][$j]['id'] != "" )
					continue;

				$followup = new TicketFollowup();
				$data = array('tickets_id' => $params['tickets'][$i]['followups'][$j]['tickets_id'],
							'requesttypes_id' => $requesttypes_id,
							'is_private' => 1,
							'content'    => addslashes($params['tickets'][$i]['followups'][$j]["content"]),
							'users_id'   => $params['tickets'][$i]['followups'][$j]['users_id'],
							'date'   => $params['tickets'][$i]['followups'][$j]['date'],
							'_no_notif'  => true,
							'_fromMobile' => true
							);
				$followup->add($data);
			...
			//em qualquer caso, importa o log do Mobidesk pro Supridesk
			for( $j = 0; $j < count($params['tickets'][$i]['log']); $j++)
			{
				$status = array('new'     => $LANG['joblist'][9],
				...
				$changes[0] = $params['tickets'][$i]['log'][$j]['id_search_option'];
				$changes[1] = $status[ $params['tickets'][$i]['log'][$j]['old_value'] ];
				$changes[2] = $status[ str_replace("'", "", $params['tickets'][$i]['log'][$j]['new_value']) ];
				Log::history( $items_id, 'Ticket', $changes, '', '0', str_replace("'", "", $params['tickets'][$i]['log'][$j]['date_mod']) );
			}
		}

		return self::methodListTickets($params, $protocol);
   }
   ...
   **/
   static function methodListTickets($params, $protocol) {
      ...
      if (isset($params['mine'])) {
		...
		/*//SUPRISERVICE*/
      // Mybox
      if (isset($params['mybox'])) {
         $where = " AND (`glpi_tickets_users_assign`.`users_id` = '".Session::getLoginUserID() . "')";
      }
      // entity_data
      if (isset($params['entity_data'])) {
         $select = " , glpi_entitydatas.address ";
         $join .= " LEFT JOIN `glpi_entitydatas`
                         ON (`glpi_tickets`.`entities_id` = `glpi_entitydatas`.`entities_id` ) ";
      }
      ...
      // Status
      if (isset($params['status'])) {
         if (!in_array($params['status'], array('all', 'assign', 'closed', 'new', 'notclosed',
                                                'notold', 'old', 'plan', 'process', 'solved',
                                                'waiting'))) {
         ...
         switch ($params['status']) {
            ...
            case 'notclosed' :
               $where .= " AND `glpi_tickets`.`status`
                                 IN ('new', 'plan', 'assign', 'waiting', 'solved')";
               break;

            case 'notold' :
               $where .= " AND `glpi_tickets`.`status` IN ('new', 'plan', 'assign', 'waiting', 'waiting1', 'waiting4', 'atendimento', 'approbation1')";
               break;
         ...
         foreach ($DB->request($query) as $data) {
            ...
            if (isset($params['id2name'])) {
               ...
               $data['entities_name']
					...
					$entitydatas->getFromDB($data['entities_id']);
               foreach ($entitydatas->fields as $key => $val) {
                  $data['entities_data'][$key] = $val;
               }
				$data['followup'] = TicketFollowup::showShortForTicket($data['id']);
								
				$data['followup_rs'] = TicketFollowup::listForTicket($data['id']);	
