...
class PluginMreportingCommon {
   ...
   function showGraph($opt, $export = false) {
      ...
      $datas = $obj->$opt['f_name']();
		...
		/*//SUPRISERVICE*/
		//se retorno de dados do gráfico for nulo, não imprime gráfico
		if ( $datas === null ) return;

		/*//SUPRISERVICE*/
		$show_label = 'hover';
		if ( array_key_exists( 'options', $datas ) && array_key_exists( 'force_show_label', $datas['options'] ) )
			if ( $datas['options']['force_show_label'] )
				$show_label = 'always'; 
			else if ($export) $show_label = 'always';
