...
class PluginMreportingGraph {
    ...
    function initGraph($title, $desc = '', $rand = '', $export = false, $delay = 365, $options = null) {
        ...
        if (!$export) {
            ...
            /* //SUPRISERVICE */
            //implementação dos parâmetros para carregar outras opções de filtro
            $showPrinterModels = is_array($options) && array_key_exists('showPrinterModels', $options) && $options['showPrinterModels'];
            $showComputerModels = is_array($options) && array_key_exists('showComputerModels', $options) && $options['showComputerModels'];
            $showGroup = is_array($options) && array_key_exists('showGroup', $options) && $options['showGroup'];
            $showManufacturer = is_array($options) && array_key_exists('showManufacturer', $options) && $options['showManufacturer'];
            $showMonitorModels = is_array($options) && array_key_exists('showMonitorModels', $options) && $options['showMonitorModels'];
            $showPrinterType = is_array($options) && array_key_exists('showPrinterType', $options) && $options['showPrinterType'];
            $showComputerType = is_array($options) && array_key_exists('showComputerType', $options) && $options['showComputerType'];
            $showMonitorType = is_array($options) && array_key_exists('showMonitorType', $options) && $options['showMonitorType'];
            $showData = is_array($options) && array_key_exists('showData', $options) && $options['showData'];
            $showContracts = is_array($options) && array_key_exists('showContracts', $options) && $options['showContracts'];
            $showLocations = is_array($options) && array_key_exists('showLocations', $options) && $options['showLocations'];
            $showStatus = is_array($options) && array_key_exists('showStatus', $options) && $options['showStatus'];
            $showStates = is_array($options) && array_key_exists('showStates', $options) && $options['showStates'];
            $showTypeEquipamento = is_array($options) && array_key_exists('showTypeEquipamento', $options) && $options['showTypeEquipamento'];
            $showTypeEquipamentoVenda = is_array($options) && array_key_exists('showTypeEquipamentoVenda', $options) && $options['showTypeEquipamentoVenda'];
            $showTypeEquipamentoBackup = is_array($options) && array_key_exists('showTypeEquipamentoBackup', $options) && $options['showTypeEquipamentoBackup'];
            $showAnalistas = ( is_array($options) && array_key_exists('showAnalistas', $options) && $options['showAnalistas'] ) ? $options['showAnalistas'] : false;
            $printermodels_id = 0;
            if (isset($_REQUEST['printermodels_id']))
                $printermodels_id = $_REQUEST['printermodels_id'];
            PluginMreportingMisc::showSelector($_REQUEST['date1'], $_REQUEST['date2'], 
           array('showPrinterModels' => $showPrinterModels,
                'showComputerModels' => $showComputerModels,
                'showMonitorModels' => $showMonitorModels,
                'showManufacturer' => $showManufacturer,
                'showContracts' => $showContracts,
                'showLocations' => $showLocations,
                'showAnalistas' => $showAnalistas,
                'showStatus' => $showStatus,
                'printermodels_id' => $printermodels_id,
                'showMonitorType' => $showMonitorType,
                'showComputerType' => $showComputerType,
                'showPrinterType' => $showPrinterType,
                'showGroup' => $showGroup,
                'showTypeEquipamento' => $showTypeEquipamento,
                'showTypeEquipamentoVenda' => $showTypeEquipamentoVenda,
                'showTypeEquipamentoBackup' => $showTypeEquipamentoBackup,
    ...
    function showHbar($raw_datas, $title, $desc = "", $show_label = 'hover', $export = false) {
        ...
        /* //SUPRISERVICE */
        $options = null;
        if (isset($raw_datas['options']))
            $options = $raw_datas['options'];
        $this->initGraph($title, $desc, $rand, $export, $delay, $options);
...
JAVASCRIPT;
        /* //SUPRISERVICE */
        if (!isset($options['hide_graph']) || !$options['hide_graph']) {
            echo $JS;
            $this->endGraph($rand, $export);
        } else {
            $this->endGraph($rand, true);
        }
        //--
    }
