<?php
/*
 * @version $Id: massiveaction.php 19104 2012-08-21 07:43:03Z moyo $
 -------------------------------------------------------------------------
            ...
            if ($val == 1) {
               ...
               if ($item->can($key,'w')) {
                  if ($item->update($input)) {
							...
							//pega os usuários do ticket para excluir, exceto o que foi adicionado.
							$ticket_user = new Ticket_User();
							$users = $ticket_user->getForTickets($input['id'], $_POST['actortype']);
							...
							foreach($users as $user)
							{
								...
								$ticket_user->check($user['id'], 'w');
								$ticket_user->delete($user);
							}
                     $nbok++;
                  } else {
                     $nbko++;
