...
/*
 * @version $Id: cartridge.form.php 17954 2012-03-21 09:00:37Z moyo $
  -------------------------------------------------------------------------
...
if (isset($_POST["update_cart_use"])) {
    ...
    if (isset($_POST['date_out'])) {
        foreach ($_POST["date_out"] as $key => $value) {
            ...
            if (isset($_POST['pages'][$key])) {
                /* //SUPRISERVICE */
                if ($cart->updateCartOut($_POST['pages'][$key], $value, $_POST['tickets_id'][$key])) {
                    Event::log(0, "cartridges", 4, "inventory", $_SESSION["glpiname"] . " " . $LANG['log'][94]);
                }
            }
        }
    ...
    Log::history($_POST["tID"], 'CartridgeItem', $changes, 'CartridgeItem', Log::HISTORY_LOG_SIMPLE_MESSAGE);
    ...
    if ($cart->delete($_GET)) {
        ...
        $changes[0] = $_GET["tID"];
        $changes[1] = '';
        $changes[2] = "deletado 1 consumível.";
        Log::history($_GET["tID"], 'CartridgeItem', $changes, 'CartridgeItem', Log::HISTORY_LOG_SIMPLE_MESSAGE);

    }
    Html::back();
} else if (isset($_GET["restore"])) {
    $cart->check($_GET["tID"], 'w'); /* //SUPRISERVICE */
    ...
    //$cartype->check($_POST["tID"],'w'); /*//SUPRISERVICE*/
    $cartridgeitemUser = explode("-", $_POST["tID"]);

    if (count($cartridgeitemUser) == 1)
        $cartridgeitemUser[1] = 0;

    $tID = $cartridgeitemUser[0];
    $userID = $cartridgeitemUser[1];

    if (array_key_exists("tID", $_POST) && isset($_POST["tID"]) && $cart->install($_POST["pID"], $tID, $userID)) {
    ...
    //$cartype->check($_GET["tID"],'w'); /*//SUPRISERVICE*/
